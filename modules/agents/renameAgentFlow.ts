 ...
 *
 * Nazwa agenta jest kluczem obcym do TRZECH niezależnych bytów - plik YAML, folder pamięci na
 * dysku (`.pkm-assistant/agents/<safeName>/`) i klucze map runtime'u
 * (`agents`/`agentMemories`/`agentHistories`). Zwykły `agent.update({name})` mutuje tylko obiekt
 * w pamięci; nie sprawdza kolizji ani nie przenosi pliku/folderu na dysku - dlatego zmiana nazwy
 * ma jednego właściciela operacji (tę funkcję), a nie zwykły update().
 *
 ...
 *
 * Trzy nieoczywiste krawędzie, wszystkie obsłużone tutaj:
 ...
 *   (2) **Bramka kolizji jest fail-CLOSED.** Brak odpowiedzi adaptera na *którymkolwiek* z dwóch
 *       sprawdzeń (plik YAML / folder pamięci) to odmowa (`collision_check_failed`) - nigdy ciche
 *       domyślenie się braku kolizji.
 *   (3) **Osierocony folder pamięci pod nowym slugiem też jest kolizją.** Sam plik YAML nie
 *       wystarczy sprawdzić - folder pamięci po dawno skasowanym agencie (ten sam slug) mógłby
 *       zostać cicho wchłonięty przez `copyFolder`, mieszając cudze notatki z nowymi.
 *       `newMemoryDirTouched` pilnuje też, żeby rollback (na pad kopiowania/zapisu) kasował
 *       WYŁĄCZNIE folder, który ta operacja sama założyła.
 * Do tego skrzynka komunikatora (`.pkm-assistant/komunikator/inbox/<slug>/`) wędruje razem
...
export interface RenameAgentResult {
    ...
    filePath?: string;
    /**
     * Skrzynka komunikatora (`.pkm-assistant/komunikator/inbox/<slug>/`) to BEST-EFFORT: pad
     * przenosin NIE cofa udanego rename'u agenta (YAML+pamięć już potwierdzone). `true` = próba
     * przenosin była i nie powiodła się w całości - stara skrzynka może zostać osierocona pod
     * dawnym slugiem. Caller loguje to głośno (patrz `AgentManager.renameAgent`).
     */
...
/**
 * Skrzynka `KomunikatorManager` liczy slug identycznie (`_getSafeName`, ten sam
...
export async function renameAgentOnDisk(
    ...
    // (e) Skrzynka komunikatora - BEST-EFFORT. Rename agenta jest już policzony
    ...
    try {
        ...
        if (inboxExisted) {
            ...
            // Bramka na CEL - skrzynka po skasowanym agencie pod nowym slugiem
