/**
 * Synchronizacja pola „Model główny" (legacy `model`) z per-rolowym `models.main`.
 *
 * `AgentProfileView.ts` budował `formData` z blokiem „Sync model ↔ models.main", który - gdy
 * `models.main` był ustawiony - KOPIOWAŁ jego wartość do `formData.model` (legacy top-level
 * pole). `profile_advanced.ts`'s onChange selecta robił to samo w drugą stronę (pisał OBA pola
 * naraz). Skutek: `profile_advanced.handleSave` zawsze wysyła `updates.model = formData.model
 * || null` - więc KAŻDY zapis profilu (dowolna zakładka, niekoniecznie „Zaawansowane") dla
 * agenta z ustawionym `models.main` DOPISYWAŁ do yamla `model:` = kopię `models.main`. Gdy
 * wielu agentów dzieli ten sam `models.main`, ten mechanizm nadpisuje `model:` tą samą wartością
 * u wszystkich naraz.
 *
 * KANON: `models.main`. Legacy `model` NIGDY nie jest kopiowany Z `models.main` z powrotem -
 * tylko odwrotnie, i tylko RAZ (jednorazowa migracja starych configów, które mają TYLKO legacy
 * `model` bez `models.main`). Gdy `models.main` istnieje (świeży ALBO właśnie zmigrowany),
 * `model` w wyniku jest ZAWSZE `null` - bez wyjątku dla agentów, którzy mieli OBA pola ustawione
 * naraz (resztki starego buga); inaczej takie yamle nigdy by się nie doczyściły (patrz
 * uzasadnienie przy `resolveMainModelForForm`).
 *
 ...
 *
 * (Wcześniejsza wersja w gałęzi „oba pola ustawione" ZWRACAŁA `model: legacyModel` niezmieniony
 * - więc agent z resztkami starego buga (oba pola z tą samą wartością) nigdy by się nie
 * doczyścił: KAŻDY kolejny zapis profilu wciąż pisałby `model:` do yamla, mimo że models.main
 * jest kanonem od dawna. Bezpiecznie zerować legacy pole bezwarunkowo, bo rozstrzyganie modelu
 * dla ról sub (`modules/models/modelResolver.ts`) NIE zależy już wyłącznie od `agent.model`.)
 */
