 ...
 *
 * `access_policy_version` jest ZAWSZE w pliku nadpisań (patrz `ALWAYS_WRITTEN_META_FIELDS`
 * w `AgentLoader.ts`) - bez tego `diffAgentConfig` je wycinało (baseline ma dokładnie tę samą,
 * bieżącą wartość), więc każdy load widział `accessVersion 0 < ACCESS_POLICY_VERSION`, odpalał
 * `migrateAccessPolicy` i przepisywał plik PO KAŻDYM zapisie. Testy "plik ma TYLKO X" niżej
 * liczą się z tym jednym stałym polem.
 *
...
/** Pomija stałe meta-pole `access_policy_version`, żeby asercje "plik ma TYLKO X" nie
...
test('agent identyczny z Jaskrem fabrycznym zapisuje plik nadpisań PUSTY poza meta-polem access_policy_version', async t => {
    ...
    // `access_policy_version` jest ZAWSZE obecne (meta-pole, patrz nagłówek pliku) - reszta
    // musi być pusta.
...
test('agent = default + zmieniona JEDNA wartość → plik ma TYLKO tę wartość (+ access_policy_version)', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('zmiana emoji/color/personality nie wycieka reszty fabrycznego configu', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('pole różniące się od baseline w TABLICY (disabled_tools) jest zapisywane', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('diff-only override file jest kompatybilny z odczytem (round-trip przez loadBuiltInAgents)', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('pole nieobecne w baseline, a ustawione na agencie, jest zapisywane (fail-safe strony diffu)', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('saveAgent(jaskier) deleguje do saveBuiltInOverrides', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
test('agent zwykły (nie built-in) nadal dostaje pełny serialize(), bez diffu', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
// ─── `access_policy_version` zawsze w diffie → migracja nie odpala się już przy KAŻDYM
// kolejnym load po zapisie profilu (patrz nagłówek pliku).

test('po saveBuiltInOverrides na niezmienionym Jaskrze kolejny load NIE odpala migracji (zero zapisów, treść pliku nietknięta)', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
    ...
    // → migrateAccessPolicy). Bez tego to wołanie zapisuje plik i loguje "Migrated built-in
...
// ─── Baseline diffu jest per built-in, nie hardcoded Jaskier.

test('built-in bez zarejestrowanego baseline (agent.name spoza BUILT_IN_BASELINES) dostaje pełny serialize() — fallback, nie diff', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
...
// Ostrzeżenie "brak baseline" krzyczało przy KAŻDYM zapisie profilu tego built-ina,
// w nieskończoność - realna wartość jest w PIERWSZYM ostrzeżeniu, reszta to szum.
test.serial('log.warn „brak baseline" leci RAZ per nazwę agenta, nie przy każdym zapisie', async t => {
    const vault = makeMemoryVault();
    const loader = new AgentLoader(vault as never);
