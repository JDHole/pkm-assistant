...
├── archetypes/
│   ├── HumanVibe.ts                # config Jaskra (HUMAN_VIBE_CONFIG) + persona PL/EN - config wbudowanego agenta, nie osobny byt-archetyp
│   ├── HumanVibe.test.ts
...
│   ├── agentExportYaml.ts          # buildAgentExportYaml (guzik „Eksportuj" -> schowek) - czysty, testowalny obok profile_advanced.ts
│   ├── agentExportYaml.test.ts
...
- **Jaskier startuje z PUSTĄ listą skilli.** Plugin nie dostarcza żadnych fabrycznych skilli (decyzja właściciela 2026-09, historia w `modules/skills/CLAUDE.md`), więc `HUMAN_VIBE_CONFIG.skills` to `[]` - inaczej profil pokazywałby baner brakujących skilli każdemu nowemu userowi. U istniejących userów własna lista z `jaskier_overrides.yaml` działa bez zmian; kto nigdy nie zapisał profilu Jaskra, ma po aktualizacji pustą listę, a pliki skilli dalej leżą w bibliotece i da się je przypisać z powrotem. Persona mówi „pomogę zaprojektować" agentów i skille, nie „mogę stworzyć": zapis pod `.pkm-assistant/**` jest dla agenta bez `admin_access` chroniony (`modules/tools/vault_path_validator.ts`), więc obietnica tworzenia kończyłaby się odmową narzędzia.
...
- **Persona Jaskra idzie za językiem interfejsu.** `archetypes/HumanVibe.ts` trzyma DWA warianty (`PERSONALITY_PL`/`PERSONALITY_EN`, oba blisko configu - to prompt systemowy, nie napis UI, więc świadomie poza słownikiem `t()`); wybiera `createJaskier()` przez `getLocale()` **przy każdym wywołaniu**, nie przy imporcie modułu (`setLocale()` leci w `src/main.ts` już po imporcie). Nadpisania usera działają bez zmian: `BUILT_IN_BASELINES['Jaskier']` woła tę samą fabrykę, więc diff liczy się względem baseline'u w aktualnym języku. Starter globalnej mapy vaulta (`VaultMap.ts`) idzie tą samą drogą, tyle że przez klucz `starter.vault_map.global` - i **tylko przy pierwszym zasiewie**: istniejącego `vault_map.md` zmiana języka nie rusza (to dokument, który agenci dopisują od pierwszej sesji).
...
- ⚠️ **Indeks artefaktów „w toku" w prompcie (`_buildBaseContext`) domyka przez `isClosedStatus`, nie przez literał PL na sztywno.** `isClosedStatus(status, statusy)` (`modules/artifacts/artifactStatuses.ts`, reeksportowany z barrela `modules/artifacts/index.js` WYŁĄCZNIE dla tego wołacza) rozpoznaje domknięcie w OBU językach (`'zamkniety'`/`'closed'`) plus fallback pozycyjny dla typu WŁASNEGO (ostatni status listy). Stare porównanie `a.status !== 'zamkniety'` zostawiałoby artefakty typu EN (status `'closed'`, bo `ensureBuiltinTypes` reseeduje NIETKNIĘTY plik typu do bieżącego języka UI przy starcie) i typu własnego (np. `'done'`) w indeksie "w toku" na zawsze, mimo że są domknięte.
