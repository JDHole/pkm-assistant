...
- `ARTIFACT_SECTION_NAMES`, `artifactSection(key, locale?)` - nazwy sekcji wbudowanych typów w obu językach. W barrelu, bo wypisują je także `modules/tools/toolAliases.ts` (aliasy `plan_review` → `heading: steps`, `idea_review` → `heading: content`) oraz `modules/prompts` (`decisionTree.ts` wypełnia placeholdery `{{user_notes}}`/`{{steps}}`… w regułach, `artifactIndex.ts` - nazwę chronionej sekcji w `prompt.dt.active_artifact`). Teksty fabryczne typów są statyczne i rejestru nie wołają - ich nagłówki pilnuje test drift-guard
...
- `activateArtifactInChat` - ciche przypięcie artefaktu do rozmowy (ustawia aktywny, **zero wysyłki**); jedyny wołacz: klik w pickerze slim bara (`chat_ui.ts`). **Nie** akcja "Otwórz" na kafelku przywołania artefaktu - ta idzie TYLKO przez `ArtifactStore.read(id)` + `plugin.openNote(path)` (`modules/chat/chat/machineTile.ts`, uwaga 4 spec A3-fix - `activateArtifactInChat` miała trzy skutki uboczne, których "Otwórz" nie potrzebuje: przypięcie artefaktu, odsłonięcie panelu, przełączenie widoku)
...
- ⚠️ **`_buildInstancePathSync` nigdy nie zwraca `null` - każdy sufiks kolizyjny idzie przez tę samą walidację co ścieżka bazowa.** `sanitizePath` zwraca `string | null`; sufiks " 2"/" 3"… NIE jest z natury bezpieczny, bo dopycha nazwę segmentu w górę - tytuł mieszczący się dokładnie na granicy `MAX_SEGMENT_LENGTH` (255 znaków) przechodzi bez sufiksu, ale z " 2" przebija limit. Wcześniej pętla przypisywała wynik `sanitizePath(...)` do `path` bez sprawdzenia i kończyła `return path as string` - `create()` woła potem `path.slice(...)` bez straży, więc kolizja na granicy długości kończyła się gołym `TypeError`, nie czytelnym błędem. Dziś każdy kandydat (bazowy i sufiksowany) idzie przez jeden `validated()`, który rzuca `'Nie udało się zbudować bezpiecznej ścieżki artefaktu'` zamiast wpuścić `null` dalej - `instancePathFor` łapie ten wyjątek i tak jak dotąd oddaje `null` (bramka nie ma prawa rzucać).
...
- ⚠️ **`pkm-artefakt` porównuj/kluczuj/ZWRACAJ ZAWSZE po Stringu.** YAML bez cudzysłowu (`pkm-artefakt: 20260911`) parsuje na `number` - `_ensureRegistry` kluczował rejestr surowym `id` (`registry.set(id as string, …)`, cast na wiarę bez `String()`), a `_pathIndexEntryValid`/`_rescanForId`/`_diskFallbackForId` porównywały `fm['pkm-artefakt'] === id` (string) strict-equal - obie drogi gubiły numeryczny artefakt: rejestr kluczowany liczbą nie trafiał w zapytanie stringiem, a nawet po naprawieniu klucza walidacja wpisu i samoleczenie dalej odrzucały go jako "obcy" (`20260911 === '20260911'` to `false`). **Piąte miejsce jest inne z natury - `_toThin` (kształt `artifact_create`/`read`/`update` zwracany MODELOWI) zwracał `id` SUROWE.** Model dosłownie ECHUJE to, co dostał - numer zamiast stringa w `artifact_read`/`create`/`update` wracałby w kolejnym wywołaniu jako JSON `number`, a `artifactTargetPath` (`modules/tools/built-in-servers/artifacts/ArtifactReadTool.ts`, `typeof id !== 'string'`) liczy wtedy PUSTY cel - bramka odmawia fail-closed, mimo że artefakt naprawdę istnieje. Naprawa jest w PIĘCIU miejscach naraz (`_ensureRegistry`, `_pathIndexEntryValid`, `_rescanForId`, `_diskFallbackForId`, `_toThin`) - `_indexSingleFile` już robił `String(id)` poprawnie i posłużył za wzór dla pierwszych czterech. **Dokładając kolejne miejsce, które czyta `fm['pkm-artefakt']` (do porównania, klucza ALBO zwrotki do modelu), normalizuj przez `String()` - nie ufaj, że YAML oddał string.**
...
- ⚠️ **`summonAgentForArtifact` łapie odrzucenie `send_message`, nie tylko synchroniczny throw.** `view.send_message` jest asynchroniczna - `try/catch` sam osłania WYŁĄCZNIE throw sprzed zwrotu promisy. Realny pad (model niekonfigurowany, brak klucza API) wraca jako odrzucona promisa i bez `.catch` uciekał jako unhandled rejection zamiast zginąć cicho tak samo jak throw synchroniczny. `artifactSummon.ts` sprawdza, czy wynik jest thenable, i loguje przez `log.warn` zamiast pustego `catch {}` - zachowanie usera (wiadomość i tak trafiła do inputu) się nie zmienia, znika tylko unhandled rejection.
- ⚠️ **Przywołanie niesie `machineMeta({_artifactSummon: true})`, nie goły `MACHINE_MESSAGE_META`** (2.3.0, spec A3 "Czat bez ścian"). Ten dodatkowy znacznik czyta klasyfikator wiadomości maszynowych w czacie (`modules/chat/chat/machineMessage.ts`) - renderuje tę wiadomość jako zwijalny kafelek systemowy zamiast dymka usera, mimo że `role` zostaje `'user'` dla modelu (zasada nadrzędna A3: treść dla modelu bez zmian, `buildSummonMessage` nietknięty). `machineMeta` dokłada `origin:'machine'` JAKO OSTATNIE pole, więc proweniencja zostaje maszynowa niezależnie od tego, co dodasz do `extra`.
