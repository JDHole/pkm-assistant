/**
 * Strażnik: `_genericVaultMap` i `compileVaultMap` czytają wpisy
...
test('_genericVaultMap: wpis string trafia na listę bez zmian', t => {
...
test('_genericVaultMap: wpis {path,access} pokazuje ścieżkę, nie [object Object]', t => {
    const pm = makeManager() as never as { _genericVaultMap: (agent: { name: string; focusFolders?: unknown[] }) => string };
...
test('_genericVaultMap: wpis {group} pokazuje etykietę grupy, nie [object Object]', t => {
    const pm = makeManager() as never as { _genericVaultMap: (agent: { name: string; focusFolders?: unknown[] }) => string };
...
test('compileVaultMap: wpis {group} w focusFolders nie produkuje undefined/ na whiteliście', async t => {
