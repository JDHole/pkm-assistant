...
test('SaveSessionWorkflow proposes a brain note from "pamietaj ze" session text', t => {
    ...
    const notes = workflow.proposeNotes([
        { role: 'user', content: 'pamiętaj że Jan lubi krótkie raporty.' }
    ], [], 'pl');
    ...
    t.like(notes[0], {
        type: 'user',
        section: '## User',
        content: 'Jan lubi krótkie raporty.'
    });
});
...
test('prepareProposals: sekcja notatki to ## Bieżące, gdy istniejący brain.md agenta jest PL', async t => {
    ...
    t.is(prep.notes[0].section, '## Bieżące');
});

// Brak brain.md na dysku - rodzi się w JĘZYKU INTERFEJSU (jak nowy agent); test przypina PL.
test.serial('SaveSessionWorkflow refreshes brain.md as an index instead of appending raw facts', async t => {
    ...
    await workflow._createBrainNote({
        ...
        description: 'Jan chce polski styl bez emoji.',
        type: 'agent_rule',
...
test('SaveSessionWorkflow creates accepted brain note and archives active session', async t => {
    ...
    t.true(files[notePath].includes('Jan lubi krótkie raporty.'));
    t.true(files['.pkm-assistant/agents/jaskier/memory/brain.md'].includes('[[brain/user_jan_lubi_krotkie_raporty.md]]'));
...
test('_createBrainNote dokłada kanoniczną stopkę Why/How (jak memory_save)', async t => {
    ...
    const created = await workflow._createBrainNote({
        ...
        why: 'Bo Jan o to prosił.',
        how_to_apply: 'Użyj przy planowaniu sesji.',
    });

    const body = files[created.path];
    // Stopka Why/How musi trafiać na dysk mimo że LLM w `/save session` generuje
    // `why`/`how_to_apply` (workPrompts.ts) — bez wspólnej ścieżki zapisu
    // (`AgentMemory.writeBrainNote`) łatwo to zgubić.
    t.true(body.includes(`${tr('memory.note.why_label')} Bo Jan o to prosił.`), 'stopka „Dlaczego" niesie treść z pola why');
...
test('_createBrainNote escapuje frontmatter przez JSON.stringify — dwukropek w name/description przeżywa zapis', async t => {
    const { vault, files } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    const workflow = new SaveSessionWorkflow(memory);
    ...
    // Escapowanie frontmattera musi iść przez `JSON.stringify` (kanoniczny pisarz,
    // `AgentMemory._buildBrainNoteContent`), nie przez prostą podmianę dwukropka — inaczej
    // „spotkanie 14:30" wraca na dysku jako „spotkanie 14 -30", bezpowrotnie.
...
test('pad zapisu JEDNEJ notatki nie blokuje pozostałych, przebudowy indeksu ani archiwizacji sesji', async t => {
    const { vault, files } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    const path = await memory.appendToActiveSession({
        type: 'user_message',
...
// ─── Usage strzalu propozycji wychodzi z prepareProposals (zeby ktos mogl zaksiegowac koszt) ───

test('prepareProposals oddaje usage ze strzalu LLM', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    await memory.appendToActiveSession({
        type: 'user_message',
...
test('sciezka regexowa (bez modelu) nie obiecuje usage', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    await memory.appendToActiveSession({
        type: 'user_message',
...
test('SaveSessionWorkflow uses LLM via factory save prompt when agent has none', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    await memory.appendToActiveSession({
        type: 'user_message',
...
// ─── „Na teraz" short-term updates from the LLM proposal ───

test('LLM na_teraz block parses into ADD/DELETE updates', t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    const workflow = new SaveSessionWorkflow(memory);
    const parsed = workflow._parseAgentJsonResponse(JSON.stringify({
        new_notes: [],
        na_teraz: {
            user: { add: ['Jan testuje panel'], remove: ['Stary stan usera'] },
            environment: { add: [], remove: [] }
        }
    }));
    t.is(parsed.na_teraz_updates.length, 2);
    t.like(parsed.na_teraz_updates.find(u => u.action === 'ADD'), { section: 'user', content: 'Jan testuje panel' });
    t.like(parsed.na_teraz_updates.find(u => u.action === 'DELETE'), { section: 'user', oldContent: 'Stary stan usera' });
});

test('applyDecision applies accepted „Na teraz" updates via the writer', async t => {
    const { vault, files } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    ...
    const decision = {
        ...
        notes: [],
        brainUpdates: [
            { action: 'ADD', section: 'user', content: 'Jan testuje panel', accepted: true },
    ...
    const brain = files['.pkm-assistant/agents/jaskier/memory/brain.md'];
    t.true(brain.includes('- Jan testuje panel'));
...
// ─── prepareProposals przekazuje opcje streamu (onChunk / signal / watchdog) ───
...
test('prepareProposals karmi onChunk deltami ze streamu (znak życia dla modalu)', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
...
test('prepareProposals honoruje AbortSignal — „Anuluj" nie udaje propozycji z regexów', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    ...
    const pending = workflow.prepareProposals(
        { path: 'x.md', messages: [{ role: 'user', content: 'pamiętaj że Jan lubi krótkie raporty.' }] },
...
test('zwykła awaria modelu NADAL spada cicho na regexy (bez zmian dla starych wywołań)', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    const workflow = new SaveSessionWorkflow(memory, {
        agent: { name: 'Jaskier', save_session_prompt: FAKE_PROMPT },
    ...
    const prep = await workflow.prepareProposals({
        path: 'x.md',
        messages: [{ role: 'user', content: 'pamiętaj że Jan lubi krótkie raporty.' }],
    });
...
// ─── Poczekalnia rescue dołączona do propozycji /save session ───
//
// Kandydaci z brain/pending_rescue/ dołączają do TEJ SAMEJ listy `notes`, którą już renderuje
// SaveSessionModal, zamiast trafiać do brain/ bez review — accept tworzy notatkę (przez
// AgentMemory.acceptPendingRescue), reject i anulowanie modala mają swoje własne, odrębne
// zachowanie (patrz testy niżej).

test('prepareProposals dokłada kandydatów z poczekalni do listy notes', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    ...
    t.is(prep.notes[0].pendingFilename, 'user_jan_lubi_ciemny_motyw.md');
    t.is(prep.notes[0].content, 'Jan lubi ciemny motyw w Obsidianie.');
...
test('pending i zwykłe propozycje regexowe współistnieją na jednej liście', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    await memory.writePendingRescue({ name: 'z rescue', type: 'reference', content: 'kandydat z kompresji' });
    await memory.appendToActiveSession({
        type: 'user_message', content: 'pamiętaj że Jan lubi krótkie raporty.',
    ...
    t.true(prep.notes.some(n => !n.pendingFilename && n.content === 'Jan lubi krótkie raporty.'));
});

test('padnięte listPendingRescue nie wywala prepareProposals — po prostu brak kandydatów w tej rundzie', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
...
test('applyDecision reject notatki-pending (accepted:false) kasuje kandydata, brain/ bez zmian', async t => {
    const { vault, files } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
...
test('anulowanie CAŁEGO modala zostawia poczekalnię nietkniętą', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
...
test('user edytował opis w modalu — accept zapisuje wersję usera, nie prefiks pochodzenia', async t => {
    const { vault, files } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
...
// ─── Pola pending przeżywają DOKŁADNIE spread SaveSessionModal ───

test('propozycja pending przeżywa dokładnie ten spread, którego używa SaveSessionModal.setProposals', async t => {
...
test('propozycja BEZ pendingFilename (rekonstrukcja pól a la _normalizeUpdate) tworzy DUPLIKAT i osiera kandydata — dokumentuje ryzyko z komentarza w SessionNote', async t => {
...
// ─── Kandydat bez name/filename nie wywala całego /save session ───

test('kandydat bez pól (obca implementacja listPendingRescue) nie wywala prepareProposals', async t => {
    const { vault } = makeVault();
    const memory = asMemory(new AgentMemory(vault, 'Jaskier'));
    await memory.appendToActiveSession({
        type: 'user_message', content: 'pamiętaj że Jan lubi krótkie raporty.',
    ...
    // name i BEZ filename. `p.name || p.filename.replace(...)` rzuciłby TypeError bez tego
    // zabezpieczenia.
    ...
    t.is(prep.notes[0].content, 'Jan lubi krótkie raporty.');
});
...
// ── Frontmatter notatki musi przeżyć YAML parser pluginu ──────────
//
// Na 248 notatek pamięci załogi 9 miało frontmatter, którego YAML nie
...
//
// `SaveSessionWorkflow` pisze nagłówek notatki z save_session przez `JSON.stringify`
// (ta sama szkoła co `AgentMemory`), więc ten test pilnuje regresji.
