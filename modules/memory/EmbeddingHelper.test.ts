/**
 * EmbeddingHelper.test.ts (`modules/embedding/contracts.ts` §6).
 *
 * Wyrównanie wyniku z wejściem oraz filtrowanie pustych tekstów NIE są robotą helpera - to
 * kontrakt `EmbeddingModel.embed()` (N wejść → N wyników, `vector:null` dla wejścia pustego,
 * zero żądań przy samych pustych wejściach) trzyma wyrównanie SAM. Helper tylko DELEGUJE
 * (`modules/memory/EmbeddingHelper.ts::embedBatch`).
 */
...
test('embedBatch: N wejść -> N wyników; helper NIE filtruje (kontrakt modelu, nie helpera)', async t => {
    ...
    // Helper deleguje WPROST do modelu - to model decyduje, co robić z pustymi
    // wejściami. Helper przekazuje WSZYSTKIE teksty, bez własnego filtrowania.
...
test('embed() bez gotowego modelu rzuca "Embed model not ready"', async t => {
...
// ── cosineSimilarity: przypadki brzegowe ─────────────────────────────────────

test('cosineSimilarity — różna długość / null / wektor zerowy -> 0; identyczne -> 1', t => {
    const helper = new EmbeddingHelper(null);
