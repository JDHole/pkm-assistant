 ...
 *
 * 24 symbole bez ANI JEDNEGO konsumenta spoza modułu
...
// Helpery list* wyciete (konsumenci wolaja METODY agentMemory);
...
// Pure idle-consolidation decision helper (wired in modules/chat).
...
export {
    ...
    STEP_STATUS,
    STEP_KIND,
    // normalizeUsage jest w barrelu, bo konsumuje go
    // modules/chat/slash-commands/save_session.js (wpis kosztu).
    normalizeUsage,
} from './ConsolidationRun.js';
// Jedno liczydło progów konsolidacji (`consolidationStatus.ts`, sygnatury zamrożone dla
// harnessu - patrz gotcha w CLAUDE.md tego modułu). `resolveConsolidationThresholds` i
// `CONSOLIDATION_DEFAULTS` mają konsumenta SPOZA modułu (`consolidationRunner.ts` w
// modules/chat - liczy próg dedupu jednym liczydłem zamiast własnej formuły), więc idą przez
// barrel; `planAutoConsolidation`/`resolveAutoConsolidationPolicy` zostają WEWNĄTRZ modułu
// (jedyny wołacz: `SaveSessionWorkflow`, deep-import).
export { resolveConsolidationThresholds, CONSOLIDATION_DEFAULTS } from './consolidationStatus.js';
...
// memory_list_summaries + memory_read_summary wchłonięte przez `list`/`read` (scope=memory).
...
// Cost log (koszt operacji memory / sub-agent).
export { CostLog } from './CostLog.js';
// ContextSessionGenerator (artefakt „Kontekst sesji") nie istnieje - ruch przejęły
// propozycje „Na teraz" w brain.md. Slot promptu brief_prompt (razem z DEFAULT_BRIEF_PROMPT)
// WYCIĘTY z Ustawień - nie miał ani jednego
// czytelnika (widmowy, obiecywał działanie w opisie i18n, którego nie było).
// 3-warstwowy retrieval (struktura → tekst → semantic).
export { RetrievalEngine } from './RetrievalEngine.js';

// streamToCompleteWithTools PRZENIESIONA do modules/agent-loop (runAgentLoop).
// `streamToComplete` przyjmuje opcjonalne `{onChunk, signal, watchdog}`; kody błędów
...
// Factory work-prompts owned by memory (moved from modules/agents/archetypes/savePrompts.js).
...
/** Progi konsolidacji (`resolveConsolidationThresholds`) - patrz `consolidationStatus.ts`. */
export type { ConsolidationThresholds, BrainNotesLimitSource } from './consolidationStatus.js';
