...
export class MigrationV3 {
    ...
    async needsMigration(): Promise<boolean> {
        ...
        // Brak folderu `brain/` NIE znaczy automatycznie
    ...
    buildPlan(brainContent: string): MigrationPlan {
        ...
        for (const section of sections) {
            ...
            if (SECTION_TYPES.has(key)) {
                ...
                // Sekcja ROZPOZNANA, ale bez treści (zero bloków) jest policzona
            ...
            // KAŻDA pozostała sekcja musi być policzona w
        ...
        // Dump „Legacy brain dump" tylko przy REALNEJ treści - sam szkielet
    ...
    parseSections(brainContent: string): MigrationSection[] {
        ...
        sections.push(current);
        // Porównanie z sentinelem, nie z literałem 'Preamble' - realna sekcja usera
    ...
    // `originalBrain` wraca jako drugi argument - bez niej
    ...
    // przed backupem.
    // FAIL-CLOSED zamiast cichego ignorowania.
    ...
    async applyPlan(plan: MigrationPlan, originalBrain = ''): Promise<{ notesCreated: string[]; deletedSections: string[] }> {
        ...
        for (const note of plan.notes || []) {
            ...
            // Jak w AgentMemory - „nie wiem" liczy się jako ZAJĘTĄ, żeby
    ...
    // Jednostka migracji to AKAPIT/BULLET, nie linia.
    ...
    // `originalBrain` idzie tu jako ARGUMENT, nie
 ...
 *
 * Pojedynczy słaby sygnał fałszywie wygaszał
...
/** Blok = jedna jednostka migracji (akapit/bullet); `prefix` = tekst nagłówka H3-H6, który go poprzedzał. */
...
/**
 * Grupuje linie sekcji w bloki - jednostka migracji to
...
/** Sklej blok linii w jedną myśl: `cleanLegacyLine` per linia, spacja jako separator, redukcja wielospacji. Prefix nagłówka idzie z przodu jako `"<prefix>: <flat>"`. */
...
// Kolizja nazwy dostaje sufiks (`_2`, `_3`, …)
