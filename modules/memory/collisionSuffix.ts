 ...
 * collisionSuffix.ts.
 *
 * JEDNO źródło prawdy dla wzorca „znajdź wolną nazwę pliku przy kolizji", wcześniej
 * powielonego SZEŚĆ razy w `AgentMemory.ts` jako sześć identycznych pętli (`startActiveSession`,
 * `discardActiveSession`, `saveSession`, `writeBrainNote`, `writePendingRescue`,
 * `archiveBrainNote`). Mutacyjne testy potwierdziły: podniesienie progu `suffix > 50` w
 * PIĘCIU z sześciu kopii naraz (`throw` zamiast `break`, dokładnie ten błąd, jaki kiedyś miał
 * `ArchiveWorkflow.applyDedup`) zostawiało CAŁY
 * pakiet testów zielonym - strażnik przed cichym zapisem nad cudzym plikiem nie miał
 * pokrycia strony „blokuje" dla pięciu z sześciu kopii.
 *
 * Kontrakt: `probeFile` traktuje `'unknown'` (padnięty odczyt - dyski
 ...
 * tekstu wchodzącego do ewentualnej slugifikacji U WOŁAJĄCEGO - dla nazw
