...
`_fullScan()` (wołany z pierwszego `initialize()` i z `rebuild()`) RZUCA, jeśli po całym skanie `_mtimes` zostaje pusta mimo że plików do zaindeksowania było więcej niż zero - to znaczy, że KAŻDY plik skończył trwale pominięty (np. zły klucz/model dostawcy), nie że vault jest legalnie pusty. Rzut trafia do `initialize()`'s zwykłej obsługi błędu (status `error` + automatyczne ponowienie skanu) przy pierwszym biegu, a do `rebuild()`'s `catch` przy Reindeksie - ten `catch` przywraca CAŁY poprzedni stan (`db`/`mtimes`/`dims`/`rows`/`segments`/`pending`) i NIGDY nie dochodzi do kodu sprzątającego stare segmenty (ten kod stoi w `try`, PO `_fullScan()`). Bez tego rzutu Reindeks z zerowym wynikiem committowałby PUSTĄ metę i kasował segmenty, na które STARA (wciąż aktualna) meta na dysku dalej wskazuje - żywy indeks znikałby tylko dlatego, że dostawca akurat odrzucił wszystko.
...
- `VaultIndexer.test.ts` - skan/wykluczenia, semantic smoke, restore/resync v2, kompakcja, sieroty (oba kształty `list()`), migracja v1→v2 (sukces, pad zapisu segmentu, pad zapisu META v2, pad weryfikacji - wszystkie trzy z przywróceniem meta v1), rebuild z zerowym wynikiem nie kasuje starego indeksu, wykryty rebuild (dims/model, dedupe powiadomienia przy oscylacji, rebuild wywołany z resyncu który SAM pada), ponowienie skanu po padzie w połowie (świeża baza, nie częściowa), hooki, kontrakt błędu, dispose() w trakcie flusha bez nowego timera, **współbieżność `_persistNow`** (adapter blokujący `writeBinary`/`write` na żądanie - persist w toku + flush/delete/emptied/drugi persist/rebuild w oknie `await`). Asercje na bajtach z dysku (`decodeSegment` + `Math.fround` wobec ostatniego wektora embeddera) TAM, gdzie jest co dekodować - `F1/R1`, `F1/R2`, `k`, `d`; dla notatki, której wiersz po naprawie ma być NIEOBECNY (wyczyszczona/usunięta w oknie) dowód jest inny: brak `rows[path]` w meta + wyszukiwanie semantyczne w RAM na STARYM wektorze nie trafia w tę ścieżkę (nie ma czego dekodować, skoro wiersza celowo nie ma).
