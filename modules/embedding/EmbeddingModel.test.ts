/**
 * EmbeddingModel.test.ts - kontrakt modelu embeddingu (`contracts.ts` §6).
 *
...
test('sufit czasu: default w widełkach, deps.timeoutMs nadpisuje, 0 wraca na default', t => {
    ...
    t.is(withZero.timeoutMs, DEFAULT_EMBED_TIMEOUT_MS, '0 NIGDY nie znaczy „brak limitu”');
});
...
test('za DUŻO wektorów też jest awarią kształtu (`4 vectors for 3 inputs`)', async t => {
    const texts = ['a', 'b', 'c'];
...
test('N wejść → N wyników; same puste wejścia → same `{vector:null}` i ZERO żądań', async t => {
    ...
    t.is(calls.length, 0, 'same puste wejścia -> zero żądań HTTP (oszczędność kosztu)');
});
...
test('brak klucza API kończy się rzutem PRZED żądaniem (`API key not set`)', async t => {
...
test('dostawca lokalny (needsApiKey:false) embeduje bez klucza, bez nagłówka Authorization', async t => {
...
// ── pusty tekst w środku listy ────────────────────────────────────────────────

test('pusty tekst W ŚRODKU listy nie przesuwa wyników', async t => {
...
test('429 z nagłówkiem Retry-After czeka tyle, ile każe dostawca (nie backoff wykładniczy)', async t => {
    const sleeps: number[] = [];
...
test('odpowiedź 200 z niepoprawnym JSON-em RZUCA (shape), nie wywala się gołym błędem parsera', async t => {
    const http: HttpClient = {
        async send() {
...
// ── komunikat błędu bez klucza ────────────────────────────────────────────────

test('komunikat błędu nie niesie klucza API ani nagłówków', async t => {
...
test('modelKey to "<provider>:<model>"', t => {
    const model = new EmbeddingModel(baseDeps());
...
test('dims zostaje na DEFAULT_VECTOR_DIM=1024, nawet gdy katalog dostawcy zna inny wymiar', t => {
    const model = new EmbeddingModel(baseDeps({ ctx: makeCtx({ modelId: 'text-embedding-3-small' }) }));
    t.is(model.dims, 1024);
});

// ═══════════════════════════════════════════════════════════════════════════
// Bramka mutacyjna (mutanty, które przeżyły zestaw wyżej).
...
test('świeży model startuje z backoffFactor === 1 (jeszcze przed pierwszym żądaniem)', t => {
    const model = new EmbeddingModel(baseDeps());
    t.is(model.backoffFactor, 1);
});
...
test('countTokens deleguje do dostawcy i oddaje LICZBĘ', t => {
    const model = new EmbeddingModel(baseDeps());
...
test('świeży model i model po sukcesie NIE odrabiają żadnego długu (zero uśpień)', async t => {
    const sleeps: number[] = [];
...
test('dług po wyczerpanej rundzie 429 jest odrabiany CO DO MILISEKUNDY (także dług = 1 ms)', async t => {
    const sleeps: number[] = [];
    let limit = true;
    const { http } = fakeHttp(() => (
        limit
...
test('bez Retry-After odczekania rosną wykładniczo: base × 2^runda × factor', async t => {
    const sleeps: number[] = [];
    const { http } = fakeHttp(() => jsonResponse(429, { error: { message: 'rate limited' } }));
    const model = new EmbeddingModel(baseDeps({
        http,
...
test('`attempts` liczy ŻĄDANIA — 0 bez klucza, 1 przy 500, 4 po wyczerpanym 429', async t => {
...
// ── skąd bierze się rozstrzygnięcie odpowiedzi ───────────────────────────────

test('500 z ciałem BEZ pola `error` to nadal awaria API (status decyduje), nie kształt', async t => {
...
test('200 z polem `error` w ciele to awaria API (limit zgłoszony ciałem)', async t => {
...
// ── kod limitu bez statusu 429 ───────────────────────────────────────────────

test('dostawca może zgłosić limit KODEM `rate_limited` mimo innego statusu — ponawiamy', async t => {
...
test('klucz API wpleciony w komunikat dostawcy jest ZAMASKOWANY', async t => {
    ...
    t.true(isEmbedBatchError(err));
    if (isEmbedBatchError(err)) {
        t.false(err.message.includes(key), 'klucz nie ma prawa wyjść w komunikacie');
        t.true(err.message.includes('***'), 'w jego miejsce wchodzi zamiennik');
    }
});

test('maskowanie łapie klucz o GRANICZNEJ długości 8 znaków', async t => {
    const key = 'sk-12345';
...
test('klucz w PRZYCZYNIE (Error) jest maskowany, a cudzy błąd nie jest mutowany', async t => {
    ...
    if (cause instanceof Error) {
        t.false(cause.message.includes(key), 'klucz wycięty z przyczyny');
...
test('przyczyna BEZ klucza przechodzi tą samą tożsamością (nie ma po co jej podmieniać)', async t => {
...
test('przyczyna będąca STRINGIEM z kluczem też jest maskowana', async t => {
