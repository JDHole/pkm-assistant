...
├── AttachmentManager.ts     # załączniki w czacie: 📎 picker, drag & drop, wklejenie z schowka (obrazy → base64, tekst → kontekst, PDF → ekstrakcja) + `addTextAttachment({name, text})` (2.3.0, spec D - załącznik tekstowy bez File/Blob, wołany przez menu na zaznaczeniu czatu)
...
głównym oknie". `noteLink.ts` daje cztery eksporty:

- `isVaultNotePath(s)` - `.md`/`.canvas`/`.base`, bez protokołu, bez białych znaków na końcach,
  bez ukrytego segmentu (`.pkm-assistant/...`, `.obsidian/...` - Obsidian nie otwiera ich jako
  notatek, klik dawałby pustą kartę; recenzja C).
- `openNoteWithRegistry(path, ev)` - otwiera przez zarejestrowany opener, `false` gdy rejestru nie
  ma (wołacz decyduje o fallbacku; tak robi przycisk Otwórz karty artefaktu w `machineTile.ts`).
- `setNoteOpener(opener | null)` - rejestr MODUŁU (jeden opener aktywny naraz). `ui-components`
  ...
  wywołuje `setNoteOpener(makeNoteOpener(this.app))` w `renderView`. `makeNoteOpener(app)` jest
  fabryką na poziomie modułu `chat_ui.ts`, poza `renderView`; zwrócony callback zamyka tylko `app`
  i otwiera notatkę przez `app.workspace.openLinkText(path, '', true)` (zawsze NOWA karta w głównym
  obszarze - nigdy nie podmienia zawartości panelu czatu, z którego klik wyszedł). Sprzątania w
  `onClose` NIE ma - decyzja: jeden rejestr na plugin,
  opener zależy tylko od globalnego `app`, a dwa otwarte widoki czatu dzielą slot, więc `null`
  z jednego odbierałby klikalność drugiemu; każdy `renderView` nadpisuje poprzedni opener.
...
implementuje więc RÓWNOWAŻNĄ logikę lokalnie (`makeNoteOpener(app)` w `chat_ui.ts`, wprost przez
`app.workspace.openLinkText(path, '', true)`), świadomie BEZ rozpoznawania modyfikatorów
...
| `AttachmentManager` | Klasa. `new AttachmentManager(container, plugin, { onChange, onReject })` → `buildMessageContent()` / `hasAttachments()` / `clear()` / `addTextAttachment({name, text})`. Opcjonalne `onReject(message)` dostaje komunikat i18n dla każdej odrzuconej próby dodania pliku po zakończeniu przetwarzania listy; domyślnie jest no-op. `addTextAttachment` dodaje załącznik tekstowy tą samą drogą co `_processFile`, z limitami `MAX_ATTACHMENTS`/`MAX_TEXT_SIZE`; zwraca `{added:true}` albo `{added:false, message}` z literalnym komunikatem i18n odmowy. |
