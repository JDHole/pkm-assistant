...
- `setNoteOpener(opener | null)` - rejestr MODUŁU (jeden opener aktywny naraz). `ui-components`
  ...
  wyszedł). Sprzątania w `onClose` NIE ma - decyzja (recenzja C): jeden rejestr na plugin,
...
| `AttachmentManager` | Klasa. `new AttachmentManager(container, plugin, { onChange })` → `buildMessageContent()` / `hasAttachments()` / `clear()` / `addTextAttachment({name, text})` (2.3.0, spec D "Czat bez ścian" - dodaje załącznik tekstowy TĄ SAMĄ drogą co `_processFile` dla pliku tekstowego: limit `MAX_ATTACHMENTS`/`MAX_TEXT_SIZE` w bajtach jak dziś, ten sam chip 📄; BEZ `File`/`Blob` w sygnaturze, żeby dało się wołać z menu na zaznaczeniu czatu (`modules/chat/chat/selectionMenu.ts`) i testować bez DOM plików. Zwraca `{added:true}` albo `{added:false, message}` z literalnym komunikatem i18n odmowy - wołacz sam decyduje, czy go pokazać). |
