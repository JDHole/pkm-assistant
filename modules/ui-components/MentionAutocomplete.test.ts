/**
 * MentionAutocomplete — cache + debounce w skanie vaulta.
 *
 * Bez cache'a `_updateSuggestions()` wołałoby `plugin.app.vault.getMarkdownFiles()` (i czasem
 * `getAllLoadedFiles()` DRUGI raz) na KAŻDY znak wpisany w tokenie `@...` — pełny skan +
 * dwa `.filter()` + `.sort()` z `toLowerCase()` przeliczanym na nowo za każdym razem, czyli
 * praca O(rozmiar vaulta) w ścieżce keystroke. Zamiast tego: lista notatek/folderów budowana
 * RAZ (`_ensureCaches`, leniwie, basename/path zlowercase'owane z góry), unieważniana
 * WYŁĄCZNIE zdarzeniami vaulta (`create`/`delete`/`rename`), a reakcja na klawisz
 * debounce'owana (~50ms) — burst wielu keystroke'ów w jednym mentionie liczy się raz.
 * Dodatkowo: `_open()` renderuje tylko gdy dropdown jest właśnie tworzony (nie woła
 * `_renderItems()` bez potrzeby), a `mouseover` przebudowuje dropdown tylko gdy najechany
 * wiersz NIE jest już zaznaczony (bąbelkowanie z ikony/nazwy/ścieżki wewnątrz wiersza inaczej
 * odpalałoby to po kilka razy na jedno przejechanie myszą).
 *
...
test.serial('_updateSuggestions: 8 keystrokes inside one @mention scan the vault ONCE, not 8 times', async t => {
    await withFakeDocument(async () => {
        ...
        t.is(vault.calls.getMarkdownFilesCalls, 1, '8 keystrokeów w jednym mentionie = JEDEN skan vaulta');
...
test.serial('vault "create" event invalidates the cache — next query re-scans', async t => {
    await withFakeDocument(async () => {
...
test.serial('mouseover on an already-selected row skips the dropdown rebuild', async t => {
    await withFakeDocument(async () => {
...
// ═══════════════════════════════════════════════════════════════════════════
// Debounce vs Enter/Tab/strzałki race — nawigacja klawiaturą musi FLUSHOWAĆ
// pending debounce zamiast działać na nieświeżym `items`.
// ═══════════════════════════════════════════════════════════════════════════

test.serial('Enter picks the suggestion for the LATEST typed query, not a stale debounced one', async t => {
    await withFakeDocument(async () => {
        ...
        // 'dailyp' — type "@da", keep typing "ilyp" within the debounce window, hit Enter
...
test.serial('ArrowDown/ArrowUp also flush a pending debounce before navigating', async t => {
    await withFakeDocument(async () => {
        const files = [makeFile('Notes/daniel.md', 10), makeFile('Notes/dailyp-notes.md', 5)];
        const vault = makeFakeVault(files, []);
        const textarea = makeFakeTextarea();
...
test.serial('Enter right after a bare "@" (before the debounce fires) picks the top suggestion instead of falling through to send the message', async t => {
    await withFakeDocument(async () => {
        ...
        t.true(enterEvent._defaultPrevented, 'Enter must be intercepted — an empty `items` must never fall through to the chat and send the message');
...
test.serial('regular character keydowns do NOT flush a pending debounce', async t => {
    await withFakeDocument(async () => {
        const files = Array.from({ length: 20 }, (_, i) => makeFile(`Notes/projekt-${i}.md`, i));
        const vault = makeFakeVault(files, []);
        const textarea = makeFakeTextarea();
        ...
        // debounce flush were unconditional (top of _handleKeydown, instead of scoped to
        // the 4 navigation branches), this would resurrect the O(vault)-per-keystroke bug
        // by forcing a synchronous scan on every letter.
