...
/**
 * Gdy nazwy modelu NIE MA, ale metadane z `listModels()` są — rozstrzyga metadana
 * `ModelInfo.multimodal`, nie zgadywanie po pustym stringu.
 */
test('pusty/undefined modelKey przy obecnych metadanych modelu', t => {
  t.false(
...
/**
 * Decyzja jest 3-warstwowa — dokładne dopasowanie, potem rozmyte, potem regex. Nazwa
 * z doklejonym sufiksem daty/wersji nadal jest vision.
 */
test('warstwa rozmyta i regexowa (nazwa z sufiksem daty/wersji)', t => {
...
/**
 * `inFamily` musi rozstrzygać na TAK, gdy nazwa jest DOKŁADNIE równa nazwie
 ...
 */
test('nazwa rowna dokladnie nazwie rodziny (bez sufiksu) nadal jest vision', t => {
  t.true(
...
/**
 * `metadataFor` musi szukać wpisu, którego `id` ODPOWIADA szukanej nazwie —
 ...
 * nazwie w warstwie 2.
 */
test('metadana katalogu o INNYM id nie podszywa się pod szukaną nazwę', t => {
  t.true(
    isVisionModel({
      modelKey: 'gpt-4o',
