...
export async function saveMemoryCandidatesFor(
    ...
    for (const candidate of candidates) {
        if (mem.writePendingRescue) {
            ...
            try {
                ...
                try {
                    ...
                    log.warn('Chat', `Memory candidate: poczekalnia padła, zapis wprost do brain/ (fail-soft): ${(e as Error)?.message || String(e)}`);
                } catch (e2) {
                    log.warn('Chat', `Memory candidate save failed (poczekalnia i fallback): ${(e2 as Error)?.message || String(e2)}`);
                }
            }
            continue;
        }
        ...
        try {
            ...
            log.warn('Chat', `Memory candidate save failed: ${(e as Error)?.message || String(e)}`);
        }
    }
    if (saved > 0) {
