/**
 * chat_streaming.limits — strażnik OKABLOWANIA tury czatu: sufit wyniku suba ORAZ zatrzask
 * Stopu wchodzący do pętli.
 *
 ...
 *
 * Czego pilnuje: wynik sub-agenta jest DELIVERABLE, nie zrzutem narzędzia - `delegate`
 * i `agent_delegate` dostają w pętli głównej własny sufit `subagent_result_max_chars` (60k)
 * zamiast wspólnego `max_tool_result_length` (15k). Mechanizm per-tool w samej pętli ma test
 * behawioralny (`AgentLoop.test.ts`, „maxToolResultLengthPerTool"); tu jest pilnowane samo
 * okablowanie - trzy linie, których skasowanie cofa naprawę bez ani jednego czerwonego testu.
 *
 * Pilnuje też drugiej drogi powrotu wyniku suba do pętli: zatrzasku Stopu, który musi wejść
 * do `runAgentLoop` jako `shouldAbort`.
 */
...
test('pętla główna dostaje per-tool sufit wyniku suba (delegate + agent_delegate)', t => {
    ...
    t.true(
        ...
        'Okablowanie per-tool zniknęło z tury czatu: wynik suba wraca znowu pod wspólny sufit 15k.',
    );
    for (const narzedzie of ['delegate', 'agent_delegate']) {
        t.regex(
            limity,
            // `\b` przed nazwą jest OBOWIĄZKOWE. Bez niej wzorzec dla `delegate` dopasowywał się
            // WEWNĄTRZ linii `agent_delegate: …` (między `_` a `d` nie ma granicy słowa), więc
            // skasowanie CAŁEGO osobnego wpisu `delegate:` nie dawało czerwieni - jedna z dwóch
            // pilnowanych linii była niepilnowana.
...
// ── ZATRZASK STOPU dociera do pętli ──────────────────────────
//
// `shouldAbort: () => turnAbort.isAborted()` w wywołaniu `runAgentLoop` to JEDYNY przewód,
// którym Stop wchodzi z czatu do pętli. Sam zatrzask ma testy (`turnAbort.test.ts`), pętla ma
// testy (`AgentLoop.abort.test.ts`), scenariusz harnessa `44_stop_zatrzask` podaje `shouldAbort`
// WŁASNY (harness nie stawia ChatView) - ale ten jeden przewód nie ma ŻADNEJ innej asercji:
// podmiana na `() => false` zostawia resztę zielono, a userowi guzik Stop, który po kliknięciu
// nie zatrzymuje ani kolejnych iteracji, ani wykonania narzędzi.

test('pętla czatu dostaje zatrzask Stopu (shouldAbort z turnAbort)', t => {
...
test('`turnAbort` to uchwyt TEJ tury, sięgalny dla Stopu i zamknięcia widoku', t => {
    ...
    t.regex(body, /const turnAbort = createTurnAbort\(\);/,
        'tura przestała zakładać własny zatrzask (przerwanie jest stanem TURY, nie pola widoku)');
...
test('sufit deliverable NIE jest podmieniony na wspólny max_tool_result_length', t => {
    const limity = blokLimitowPetliGlownej();
    ...
    t.false(
        ...
        'Ktoś podmienił sufit deliverable na wspólny `max_tool_result_length` - mapa per-tool zostaje, ale naprawa znika po cichu.',
    );
});

// ── reszta okablowania bloku `limits` ────────
//
// Blok `limits: { … }` przekazywany do `runAgentLoop` ma PIĘĆ linii okablowania
// (`maxIterations`, `maxToolResultLength`, `maxToolResultLengthPerTool`, `perCallTimeoutMs`
// w tym pliku + `stallTimeoutMs` osobno, przy budowie watchdoga streamu) - strażnik wyżej
// pilnuje TYLKO jednej (`maxToolResultLengthPerTool`). Bez testów niżej, podmiana
// `maxIterations: 9999,` i `perCallTimeoutMs: 0,` zostawiałaby resztę modułu chat zielonego:
// backstop iteracji i pas ostateczny per-wywołanie mogłyby zniknąć bez ani jednego czerwonego
// testu.

test('pętla główna dostaje backstop iteracji czatu (chat_max_iterations)', t => {
    const limity = blokLimitowPetliGlownej();
    t.not(limity, '', 'Nie znaleziono bloku `limits` przy `runAgentLoop` - zmienił się kształt tury czatu.');
    ...
    t.regex(
        ...
        'Twardy backstop iteracji zniknął z okablowania pętli głównej - czat albo przestał słuchać Settings→Limity, albo stracił granicę rund tool-callingu.',
    );
});

test('pętla główna dostaje wspólny sufit wyniku narzędzia (max_tool_result_length)', t => {
    const limity = blokLimitowPetliGlownej();
...
test('pętla główna dostaje budzik per-wywołanie modelu', t => {
    const limity = blokLimitowPetliGlownej();
...
test('watchdog ciszy streamu czatu dostaje chat_stream_stall_timeout_ms', t => {
