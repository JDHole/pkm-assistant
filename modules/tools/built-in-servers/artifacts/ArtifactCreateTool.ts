/**
 * artifact_create — utwórz instancję artefaktu żywego (gatunek 1) jako notatkę vaulta.
 *
...
export function createArtifactCreateTool() {
    return {
        ...
        // Bramka dostaje ścieżkę, pod którą notatka POWSTANIE — policzoną tym samym silnikiem,
        // który zaraz ją utworzy. Pusty string zamiast prawdziwej ścieżki pozwoliłby
        // `artifact_create` pisać do vaultu z pominięciem whitelisty, No-Go i zakresu suba.
        // Tożsamość agenta bierze się WYŁĄCZNIE z runtime'u (`ctx.agentName`), nigdy z
        // `args._invocationAgentName` — ten worek dociera tu w kształcie podanym przez MODEL,
        // zanim `MCPClient.executeToolCall` go nadpisze, więc nie jest źródłem prawdy dla
        // folderu, który ocenia AccessGuard.
        ...
        execute: async (args: ArtifactCreateArgs, _app: unknown, plugin: ArtifactToolPlugin | null | undefined) => {
            ...
            // zaufany (inaczej niż w `contextExtractor` — patrz komentarz wyżej).
            const agentName = (args?._invocationAgentName as string) || plugin.agentManager?.getActiveAgent?.()?.name || 'agent';

            // Egzekwowanie typów per agent: `artifact_types` ogranicza, jakiego `typ` wolno użyć
            // w `artifact_create`, nie tylko widoczność w indeksie promptu. Lista jest OPT-IN:
            // pusta/nieustawiona = wszystkie typy (zero zmian dla profili bez ograniczenia).
            // Agent nieznaleziony w managerze = przepuszczamy (nie ma czego egzekwować).
            ...
            try {
                ...
                // Store odmówił JESZCZE PRZED zapisem (wartość pola nie przeszła bramki treści) —
                // nie ma id, nie ma pliku. Model dostaje ten sam kod błędu co przy patchu, żeby
                // mógł poprawić treść, a nie zgadywać.
                if (!res.created) {
