...
// Wsteczna kompatybilność nazw narzędzi - stare nazwy retrieval → search,
...
// Nazwa pliku notatki w modalu zgody liczona TA SAMA funkcja,
...
// Jeden filtr znaczników `_invocation*` dla wysyłki i dla podglądu w approvalu.
...
export interface MCPToolRegistryLike {
    ...
    /**
     * Oś narzędziowa agenta (disabled_tools + opt-in `mcp_servers[]`). Ta sama metoda liczy
 ...
 * `app` i `plugin` (DI z runtime'u), a fabryki mogą dołożyć `contextExtractor`.
 ...
 * w oknie zgody. Dwa pola opisują KANONIZACJĘ ścieżki (patrz `_canonicalizeToolContext`).
 */
...
/** Dodatkowe opcje wykonania (autonomia czatu + kaganiec delegacji + adres zwrotny). */
export interface ExecuteToolCallOptions {
    autonomy?: string | null;
    delegationDepth?: number;
    scopeFolders?: ScopeFolders | null;
    /**
     * Adres zwrotny wołającego (kto i z której rozmowy zleca). Klient go NIE BUDUJE -
    ...
    /**
     * Whitelista narzędzi WOŁAJĄCEGO (dla biegu suba - jego
...
interface ToolExecutionResult {
    success?: boolean;
    /** Znacznik porażki dopisywany przez normalizację (krok 8b niżej). */
    isError?: boolean;
    error?: string;
    scope?: string;
    files?: unknown[];
    results?: unknown[];
    /** Spis `artifact_list` - przechodzi przez ten sam post-filtr whitelisty co `list`/`search`. */
    artifacts?: unknown[];
    count?: number;
    total?: number;
    totalCount?: number;
...
/**
 * Ile żyje wpis w pamięci odmów (ms).
 *
 * Pamięć odmów ma jedno zadanie - nie pozwolić modelowi ponawiać w kółko akcji, której user
 * właśnie odmówił. Bez TTL nie wygasałaby NIGDY: `MCPClient` powstaje raz na cykl życia
 * pluginu, `clearDenials()` nie ma w produkcji ani jednego wołacza, a klucz narzędzia bez
 * ścieżki to `<narzędzie>::*`, czyli CAŁE narzędzie. Jedno „Odmów" na serwerze MCP odbijałoby
 * więc każde następne wywołanie - w nowej zakładce, jutro rano - bez pokazania modala, a
 * jedynym resetem byłby restart pluginu. Kwadrans pokrywa rozmowę na jeden temat i wygasa,
...
const ACTION_TYPE_MAP: Record<string, string> = {
    'ask_user': 'vault.read',
    // Prymitywy (scope vault|memory dla read/list). Ten sam actionType co legacy vault_*.
    'read': 'vault.read',
    'list': 'vault.read',
    'write': 'vault.write',
    'delete': 'vault.delete',
    'create_folder': 'vault.create_folder',
    // Legacy vault_* names (aliases → read/list/write/delete/create_folder). Zachowane w mapie
    ...
    'search': 'vault.read', // unified search (scope vault/memory)
    // Legacy retrieval names (aliases → search). Zachowane w mapie akcji na wypadek
    ...
    'delegate': 'delegate',
    // `agent_message` nie jest realnym prymitywem - wpis zostaje jako fail-safe dla starych historii
    // sesji / promptów, żeby wywołanie po nieistniejącej nazwie nie wpadło w 'unknown'.
    'agent_message': 'agent.message',
    // `agent_delegate` → `agent.message`, NIE `delegate`.
    ...
    // rozmowy niczego w świecie nie zmienia - to guzik, który klika user. Mapowanie na
    // `delegate` chowałoby wysyłkę pod przełącznikiem delegacji (domyślnie CICHY) i modal
    // opisywałby ją jako delegację zamiast powiedzieć, że coś wychodzi do cudzej skrzynki.
    // `delegate` (bieg sub-agenta) zostaje nietknięty - to inna akcja i inny skutek.
    'agent_delegate': 'agent.message',
    // Gatunek 2 - todo (jednorazówka w .pkm-assistant/; read-side, poza approvalem).
    'todo': 'vault.read',
    // Artefakty żywe (gatunek 1). create/update = zapis do folderu artefaktów (approval OFF
    ...
    // `web_read` ma WLASNY typ akcji. Wspolny `web.search` znaczylby,
    // ze jeden przelacznik "Wyszukiwanie w internecie" zdejmowalby pytanie takze z pobierania
    // adresu wskazanego przez model, a modal oglaszalby wyjscie na sieciowy adres jako wyszukiwanie.
    'web_read': 'web.read',
    'add_text_to_image': 'image.generate',
    // Komunikator v3: trzy prymitywy poczty. `kom_send` = wysyłka (agent.message),
...
export class MCPClient {
     ...
     * Wpis WYGASA (patrz `DENIAL_TTL_MS`).
     */
    ...
    /**
     * Kolejka „jedna decyzja o zgodzie naraz" per `${sessionKey}::${targetPath}` - patrz
     * `_withConsentQueue`. Wewnętrzna (żaden wołacz spoza tego pliku), więc `declare` +
     * inicjalizacja w konstruktorze, jak reszta pól tej klasy.
     */
    declare _consentQueues: Map<string, Promise<void>>;
     ...
     *
     * Wpis ma TERMIN WAŻNOŚCI. Wygasły jest po drodze usuwany, żeby mapa nie
     ...
     *
     * To jest droga NATYCHMIASTOWA - do zawołania przy przełączeniu sesji /
    ...
    /**
     * Szereguje `fn` per `queueKey` - kolejne wywołanie z TYM SAMYM kluczem czeka, aż
     * poprzednie się ROZSTRZYGNIE (fulfil ALBO reject - kolejka porządkuje KOLEJNOŚĆ, nie
     * wynik), i dopiero wtedy samo rusza. `queueKey === null` = zero szeregowania, `fn()` leci
     * od razu (dziś: brak sesji albo narzędzie inne niż write/vault_write).
     *
     * PO CO: `Promise.all` w `modules/agent-loop/AgentLoop.ts` wykonuje tool-calle JEDNEJ
     * tury RÓWNOLEGLE. Bez tej kolejki N równoległych zapisów na TĘ SAMĄ ścieżkę w TEJ SAMEJ
     * sesji sprawdzałoby `sessionWriteConsent.has()` (rekontrola u wołacza, PRZED otwarciem
     * modala) zanim PIERWSZE wywołanie w ogóle zdążyło zapytać - `executeToolCall` nie ma ani
     * jednego `await` między start wywołania a tę bramkę, więc wszystkie N tool-calli dochodzi
     * tam w TYM SAMYM ticku. Efekt bez kolejki: user dostaje N modali o TĘ SAMĄ decyzję zamiast
     * jednego. Kolejka zamienia to w: pierwsze leci od razu i (być może) pyta, każde kolejne
     * CZEKA, a po obudzeniu ma sprawdzić `has()` PONOWNIE (robi to wołacz, nie ta metoda) -
     * jeśli poprzednie właśnie nadało zgodę, korzysta z niej zamiast pytać znowu.
     *
     * Odrzucenie `fn()` (deny, wyjątek narzędzia) NIE MA PRAWA zablokować następnego wołania w
     * tej samej kolejce (deadlock po pierwszej odmowie) - `settleMarker` niżej rozwiązuje się
     * ZAWSZE, niezależnie od wyniku `run`, więc kolejny `_withConsentQueue` na ten sam klucz
     * rusza, gdy tylko POPRZEDNIE się skończy, obojętnie jak.
     *
     * @returns wynik `fn()`, albo rzuca dokładnie to, co rzuciło `fn()` - kolejka porządkuje
     *   TYLKO kiedy `fn` rusza, nigdy co zwraca.
     */
    async _withConsentQueue<T>(queueKey: string | null, fn: () => Promise<T>): Promise<T> {
        if (!queueKey) return fn();
        const previous = this._consentQueues.get(queueKey) || Promise.resolve();
        const run = previous.then(fn, fn);
        const settleMarker: Promise<void> = run.then(() => undefined, () => undefined);
        this._consentQueues.set(queueKey, settleMarker);
        try {
            return await run;
        } finally {
            // Sprzątanie: zdejmij wpis TYLKO jeśli nikt nowszy się jeszcze nie doczepił
            // (porównanie po TOŻSAMOŚCI referencji) - mapa nie ma puchnąć przez cały cykl
            // życia pluginu.
            if (this._consentQueues.get(queueKey) === settleMarker) this._consentQueues.delete(queueKey);
        }
    }
    ...
    _shouldRequestApproval({ actionType, targetPath, toolName, agent, permRequiresApproval, autonomy, args = {} }: {
        ...
        return this.plugin.permissionSystem.requiresApproval(
            ...
            autonomy,
            {
                // Dla `write` liczy się WYŁĄCZNIE `mode` - to z niego
    ...
    /**
     * Sprowadza ścieżkę vaultową do FORMY KANONICZNEJ, tak żeby
    ...
    _canonicalizeToolContext(args: ToolCallArgs, ctx: ToolContext): ToolContext {
        ...
        // `output_path` dołącza do pól-ścieżek - `add_text_to_image` oddaje bramce CEL ZAPISU,
    ...
    _extractToolContext(toolName: string, args: ToolCallArgs, agentName: string | null): ToolContext {
        ...
        // Prefer tool-defined contextExtractor (moves switch logic into each tool factory).
        ...
        if (tool && typeof tool.contextExtractor === 'function') {
            try {
                ...
                // Kanonizacja obejmuje TAKŻE tę ścieżkę - to nią chodzą dzisiejsze prymitywy
        ...
        // Narzędzia zewnętrznych serwerów MCP (ExternalMcpManager) NIE są operacjami
        ...
        if (this.plugin?.externalMcpManager?.isExternalTool?.(toolName)) {
            // Szew: targetPath EMPTY -> checkPermission/AccessGuard bez pseudo-sciezki (agent z
            ...
            // Do approvalu idą też PEŁNE argumenty wywołania - user ma zobaczyć, CO
        ...
        switch (toolName) {
            ...
            case 'memory_save': {
                ...
                // JEDNA reguła nazewnicza - ta sama, którą `MemorySaveTool`
                // liczy przy zapisie (`makeMemoryNoteFilename`: NFKD + zdjęcie diakrytyków + cięcie
                // do 80 znaków + prefiks typu). Druga, uproszczona kopia pokazywałaby userowi
                // w modalu inną nazwę pliku niż ta, która realnie powstaje.
            ...
            // Poczta - pokaż adresata + temat/treść w approvalu.
            case 'kom_send':
            ...
            // Delegacja wysyła list, więc modal ma pokazać to, co
            ...
            // przy pierwszej zmianie. Bez tematu modal mówi po prostu
            ...
            // Brak case'a dla `generate_image` jest celowy: `args.prompt` NIE jest celem akcji.
            // Gdyby cel liczył się z promptu, AccessGuard i bariera `scope.folders` suba oceniałyby
            // tekst pisany przez model (wystarczyłoby zacząć prompt od nazwy własnego folderu, żeby
            // strażnik powiedział „whitelist"), podczas gdy realny zapis idzie gdzie indziej. Cel
            // liczy `contextExtractor` narzędzia (folder zapisu z ustawień); prompt jedzie osobnym
            ...
            // idea_review/plan_review/chat_todo skasowane (aliasy remapują name PRZED tym miejscem).
     ...
     *   Missing → normalized to 'edge' (≈ dzisiejsze defaulty). Patrz autonomy.js.
     * @param opts.delegationDepth - Piętro delegacji wołającego (0/brak = główny
     ...
     * @param opts.scopeFolders - Foldery `scope.folders` sub-agenta. Dodatkowe,
     *   koniunkcyjne zawężenie ścieżek vaultowych (przecięcie rodzic∩sub) - patrz AccessGuard.
     * @param opts.origin - Adres zwrotny wołającego (`{agentName, sessionPath?, tabKey?}`).
     ...
     *
     * Znormalizowana autonomia jedzie też do args jako `_invocationAutonomy`
    ...
    async executeToolCall(
        ...
        // Kaganiec delegacji. Głębokość > 0 znaczy „wołane z wnętrza sub-agenta".
        ...
        const scopeFolders = Array.isArray(opts.scopeFolders) && opts.scopeFolders.length > 0
            ? opts.scopeFolders
            : null;
        // Klient origin'u NIE buduje - bierze go od wołacza i wstrzykuje do args.
        const origin = opts.origin || null;
        // Whitelista wołającego - przelot do args, bez interpretacji.
        ...
        try {
            ...
            // Args alias resolver (backward compat dla user MCP servers + deprecated tool inputSchemas).
            // image_path/file/filepath → path (canonical naming standard).
            ...
            // Gdy model/YAML woła starą nazwę (nieznaną w registry) → przemapuj na
            ...
            // Ten sam wzorzec co `_invocationAgentName` - zaufany znacznik od runtime'u,
            ...
            // Adres zwrotny zlecenia - ten sam wzorzec zaufanego znacznika. Brak opcji =
            ...
            // Tryb autonomii TURY jako zaufany znacznik (ten sam wzorzec).
            ...
            // Zakres folderów i whitelista narzędzi WOŁAJĄCEGO -
            ...
            //    widocznych narzędzi (`checkToolAxis`), decyduje tu o WYKONANIU. Sam filtr
            //    widoczności nie wystarczy: model, który zna nazwę wyłączonego narzędzia
            //    (stara sesja, notatka, transkrypt innego agenta), mógłby je zawołać i przejść.
            ...
            // Worek idzie tu PO nadpisaniu zaufanych znaczników
            // (`argsWithContext`), a nie surowy `args`. Gdyby `contextExtractor` widział
            // `_invocation*` w kształcie, w jakim podał je MODEL - a rodzina `artifact_*`
            // wyprowadza z tożsamości CAŁY cel akcji - bramka oceniałaby inną ścieżkę niż
            // ta, do której pisze `execute` (ono zawsze dostaje worek zaufany).
            ...
            // Ścieżki nie do uratowania (traversal, absolutna, null byte,
            ...
            // Narzędzie ma dostać DOKŁADNIE ten ciąg, który oceniła bramka.
            ...
            // (they use adapter/AgentMemory directly, not vault paths). Odczyt pamięci przez
            ...
            const sessionKey = (origin && typeof origin.sessionPath === 'string' && origin.sessionPath)
                ? origin.sessionPath
                : null;
            // Klucz kolejki (`_withConsentQueue`) - TYLKO write z sesją znaną i celem. `null` =
            // zero szeregowania (zachowanie identyczne jak dotąd dla delete/innych narzędzi i dla
            // wywołań bez `origin.sessionPath`).
            const consentQueueKey = (isWriteTool && !!args.path && sessionKey)
            ...
            // `${sessionKey}::${targetPath}` (`_withConsentQueue`) - bez niej `Promise.all` w
            // `agent-loop/AgentLoop.ts` (tool-calle JEDNEJ tury lecą RÓWNOLEGLE) pokazywałby N
            // modali o TĘ SAMĄ decyzję zamiast jednego, bo `executeToolCall` nie ma ani jednego
            // `await` między startem a tą bramką - wszystkie równoległe wywołania dochodzą tu w
            // TYM SAMYM ticku i zobaczyłyby `has()` PRZED rozstrzygnięciem pierwszego modala.
            const redirectResult = await this._withConsentQueue(consentQueueKey, async (): Promise<string | null> => {
                // Rekontrola TUŻ PRZED otwarciem modala - poprzednie wywołanie w tej samej
                // kolejce (ten sam plik + ta sama sesja) mogło właśnie nadać zgodę, kiedy TO
                // wywołanie czekało w kolejce.
                ...
                if (permResult.requiresApproval && !sessionConsentGranted) {
                    ...
                    // Trzecia ścieżka - przekieruj + kontekst. User zatrzymuje
                ...
                // Nazwa narzędzia to `write` (alias remapuje vault_write → write wcześniej;
                ...
                if (permResult.requiresApproval && !sessionConsentGranted && autonomy !== 'yolo'
                    ...
                    try {
                        ...
                        // Sprawdź unikalność old_text TAK SAMO jak WriteTool.execute
            ...
            // 7b/7c. Modale review (IdeaReviewModal → _planStore) i ArtifactProgressModal
            // (polling chat_todo) SKASOWANE. Artefakty żywe mają approval w notatce (guziki B1) i live-widok
            // `todo` w czacie. Aliasy remapują stare wywołania na artifact_create / todo.

            // 7e. Przechwytywanie wyniku generowania obrazu do modala
            // podglądu SKASOWANE - generate_image zapisuje wynik prosto do vaulta.

            // 8. Whitelist post-filtering for list/search results.
            // `search` i `list` scope=vault filtrują wyniki whitelistą agenta (jak dawny
            ...
            // `scopeFolders` tnie wyniki tak samo jak whitelista rodzica (koniunkcja) -
            ...
            // Spis artefaktów jedzie tym samym filtrem co `list`/`search`.
            ...
            // Dołącz notę o przekierowaniu ze starej nazwy na `search`.
            if (aliasNote && result && typeof result === 'object' && !Array.isArray(result)) {
                result.alias_note = aliasNote;
            }

            // 8b. JEDEN punkt normalizacji porażki. Narzędzia wbudowane
            ...
            // Ten komunikat idzie MODELOWI i do transkryptu
    ...
    /**
     * Parsuje tool_calls z odpowiedzi modelu. Deleguje do kanonicznego parsera
