...
- **`new Function` w bundlu, dawniej z Ajv wewnątrz SDK MCP, dziś ZERO wystąpień - wycięte aliasem builda.** `ExternalMcpManager._createClient` podaje `jsonSchemaValidator: new CfWorkerJsonSchemaValidator()` (`@cfworker/json-schema`, `dependencies` w `package.json`) przy tworzeniu `Client` z `@modelcontextprotocol/sdk` - ten walidator ocenia WYŁĄCZNIE `outputSchema`/`structuredContent` wyników narzędzi zewnętrznych serwerów, nigdy argumenty wejściowe (osobna ścieżka w `MCPClient`/`ToolRegistry`, nieruszona). **Sama ta opcja NIE WYSTARCZA** - SDK statycznie importuje `AjvJsonSchemaValidator` z `validation/ajv-provider.js` i konstruuje ją jako fallback w konstruktorze (`options?.jsonSchemaValidator ?? new AjvJsonSchemaValidator()`), więc bez drugiego kroku ta referencja zostaje osiągalna dla esbuild i `ajv` (z jego kompilatorem schematów przez `new Function`) i tak ląduje w bundlu, mierzone empirycznie. Drugi krok: `esbuild.js`'s `ajvProviderShimPlugin()` przechwytuje import `validation/ajv-provider.js` **tylko gdy importer leży wewnątrz `node_modules/@modelcontextprotocol/sdk`** i podmienia go na `esbuild.cfworkerValidatorShim.mjs` (plik obok `esbuild.js`), który reeksportuje `CfWorkerJsonSchemaValidator` pod tą samą nazwą `AjvJsonSchemaValidator` - SDK dalej importuje i konstruuje ją bez zmian w swoim kodzie, tyle że to teraz de facto `CfWorkerJsonSchemaValidator`. Efekt mierzony: `grep -c "new Function" dist/main.js` = 0, `grep -o 'ajv-formats\|AjvJsonSchemaValidator\|require("ajv")' dist/main.js` = puste, bundle mniejszy o ok. 121 KB (`ajv`+`ajv-formats` cięższe niż `@cfworker/json-schema`) mimo doszycia nowej zależności. Wpis `ajv` w `dependencies`/`overrides` w `package.json` ZOSTAJE - dalej pin dedupu wersji tree-wide (inne pakiety, np. narzędzia lintu, dalej ciągną własny `ajv`), nie import (`core/dead_code_zasieg.test.ts`).
  - ⚠️ **Alias zależy od WEWNĘTRZNEJ ścieżki pliku SDK, nie części jego publicznego kontraktu** (`package.json#exports`). Podbicie `@modelcontextprotocol/sdk` (nawet patch) może po cichu przenieść/przemianować `validation/ajv-provider.js`, wyłączając alias BEZ ŻADNEGO błędu builda - regex po prostu nic by nie złapał, `ajv`/`new Function` wróciłyby do bundla po cichu. **Dlatego jest strażnik**: `assertNoAjvNewFunction()` w `esbuild.js`, wołany na końcu `finishBuildPlugin`'s `onEnd` (PRZED `copyStaticArtifacts`/`deployToVaults` - w tej samej funkcji, więc rzucony wyjątek blokuje kopiowanie i deploy tego builda), liczy dosłowne `new Function` w gotowym `dist/main.js` i wywala build (esbuild zamienia rzucony wyjątek w błąd builda, `ctx.rebuild()` w `main()` odrzuca obietnicę), gdy jest ich więcej niż 0 - komunikat wskazuje alias jako pierwszego podejrzanego. Działa też przy `DESTINATION_VAULTS=""` (CI). Test strukturalny źródła: `build_kontrakt.test.ts` (root).
  - ⚠️ **Różnice semantyczne Ajv vs `@cfworker/json-schema`** (nieoceniane empirycznie na realnym serwerze MCP - patrz `modules/tools/ExternalMcpManager.test.ts` dla testu kontraktowego z fake serwerem): Ajv w tej konfiguracji (`strict:false`, `validateSchema:false`, `ajv-formats`) NIE waliduje samego schematu i akceptuje słowa kluczowe spoza specyfikacji; `@cfworker/json-schema` nie ma odpowiednika `validateSchema:false`. Draft: Ajv idzie za deklaracją schematu/config; `CfWorkerJsonSchemaValidator` bez argumentu konstruktora domyślnie zakłada `2020-12` - serwer MCP deklarujący starszy draft (`4`/`7`) może wymagać jawnego `new CfWorkerJsonSchemaValidator({ draft: '7' })` zamiast bezargumentowego wywołania użytego dziś. Oba wspierają `format: 'uri'` i podstawowe formaty JSON Schema (test kontraktowy niżej to pokrywa); zakres pełnej listy formatów `ajv-formats` vs `@cfworker/json-schema` nie był punkt po punkcie porównany. `$ref` do zewnętrznych/rekurencyjnych schematów - nieoceniane (brak w tym repo realnego serwera MCP z `outputSchema` używającym `$ref`).
...
- ⚠️ **Tożsamość agenta w `executeToolCall` jest FAIL-CLOSED, nigdy nie podmienia się cicho.** Nazwa PODANA wołaniem (`invocationAgentName` niepusty), której `getAgent` nie rozwiąże (agent usunięty w trakcie tury, stara sesja po zmianie nazwy), kończy się odmową (`perm.agent_missing`) - NIGDY przejściem na aktywnego agenta, bo to byłoby wykonanie akcji pod cudzą tożsamością. `getActiveAgent()` jest dozwolony wyłącznie w gałęzi „brak nazwy" (żaden właściciel tury nie został zadeklarowany). Brak jakiegokolwiek agenta na końcu (`perm.no_agent`) odmawia PRZED `checkPermission`, dla wszystkich narzędzi łącznie z pamięcią - `permResult` nie ma prawa zostać na domyślnym `{allowed:true}` bez oceny. Okna zgody (approval + diff) podpisują się `agent.name` (obiekt, na którym policzono bramki), nie surową nazwą z wywołania.
...
- ⚠️ **Domyślny zakres `search` (brak `scope`) to PAMIĘĆ wołającego agenta, nie vault.** Reguła (i test) żyje w JEDNEJ funkcji, `SearchTool.resolveSearchScope(args, plugin)`, wołanej z DWÓCH miejsc — `contextExtractor` (bramka `targetPath`) i `execute` (silnik) — żeby obie strony nigdy nie oceniły innego zakresu dla tego samego wywołania (ten sam wzorzec co reszta modułu: „bramka i zlew oglądają jeden ciąg"). Jawne `scope:"vault"`/`scope:"memory"` (`source:'explicit'`) zachowuje dzisiejsze bramki fail-closed bez zmian (`denied_memory`/`no_agent`); dopiero BRAK `scope` (albo nierozpoznana wartość, np. literówka `"Vault"`) przechodzi przez trzy zjazdy na `vault` (`source:'default_fallback'`), w tej kolejności:
  1. `reason:'sub_agent'` — wywołanie z wnętrza sub-agenta (`_invocationDelegationDepth > 0`, zaufany znacznik z `MCPClient.ts:709-713`/`SubAgentRunner.ts:751-752`). Sub nie ma własnej szuflady pamięci; rama suba (`modules/sub-agents/framePrompt.ts:36,47`) każe mu podać `scope="memory"` jawnie, gdy naprawdę chce pamięć rodzica.
  2. `reason:'memory_off'` — agent (dokładnie ta sama tożsamość i warunek co `denied_memory`) ma `permissions.memory === false`. Fallback na `vault` zamiast odmowy: domyślne wywołanie bez `scope` ma dalej coś znaleźć, nie zwrócić błąd.
  3. `reason:'no_memory'` — `resolveAgentMemory` nie znalazła pamięci (brak `agentManagera`/brak wpisu). Bez tego domyślne `search` kończyłoby się `no_agent`.
  Dopiero gdy żaden z trzech nie zadziała → `{scope:'memory', source:'default'}`. Wynik dostaje wtedy DODATKOWE pole `scope_hint` (`t('mcp.search.scope_hint_default_memory')`) — mówi modelowi wprost, że przeszukał TYLKO pamięć i że pusty wynik nie znaczy „notatki nie ma w vaultcie". `scope_hint` NIE pojawia się dla jawnego `scope` (obu wartości) ani dla żadnego z trzech fallbacków — wynik zostaje bajtowo identyczny jak dziś. `RetrievalEngine.runSearch()` (`modules/memory/`) ma WŁASNY, inny default (niskopoziomowy zapas gdyby ktoś go wywołał bez `scope` z pominięciem tego narzędzia) - `SearchTool` podaje mu `scope` zawsze jawnie, więc jego default nigdy nie jest w praktyce brany. Z tego samego powodu aliasy wsteczne `vault_*` w `toolAliases.ts` niosą `scope:'vault'` JAWNIE: stara nazwa obiecuje vault, a alias bez `scope` po cichu przeszukiwałby pamięć. Dokładasz nowy alias na `search` → wpisz mu `scope`.
