/**
 * `render()` must paint only the live `pkm-settings-*` class names. The `pkm-settings-v2*`
 * namespace (six names) has zero CSS rule in any of the six stylesheets in the repo - styling is
 * carried entirely by the live `pkm-settings-*` dublers added alongside them
 * (`src/styles.css:831-847`), except `pkm-settings-v2__subfields`, which has NO dubler at all and
 * must not be painted either (no replacement needed: no rule ever existed for it, and the
 * container is a bare wrapper styled by its children's own `setting-item` markup).
 *
...
test('render() never paints a pkm-settings-v2* class', async t => {
...
/** Strips /* *‍/ block comments and // line comments - the strażnik pilnuje CODE, not doc comments. */
function stripComments(src: string): string {
    return src.replace(/\/\*[\s\S]*?\*\//g, '').replace(/(^|[^:])\/\/[^\n]*/g, '$1');
}

// The CODE of SettingsRegistry.ts must never regain the string
...
// comments (including this invariant's own doc comment above) are allowed to name the retired class.
