 ...
 * — dokładnie tak jak z każdego innego modułu. Pilnuje tego ESLint
 * (`no-restricted-imports`), a nie sama konwencja.
 *
 * Kryterium wstępu do core/ (ADR 003): używane przez ≥3 moduły +
 * infrastruktura (nie feature) + zmienia się rzadko.
 *
 * ── KONTRAKT: TEN BARREL JEST NODE-SAFE ───────────────────────────────────
 ...
 *
 * ⚠️ Jednolinijkowiec `node --input-type=module -e
 * "await import('./core/index.js')"` NIE jest sprawdzianem tego kontraktu:
 ...
 *
 * ⚠️ „Dokładnie cztery" liczy TYLKO pliki core/ dotykające
...
// Konwersja glob→regex, używana wyłącznie przez `AccessGuard`.
export { globPatternToRegex } from './security/globPattern.js';
// Ogrodzenie niezaufanej treści w system prompcie (jedna funkcja dla wszystkich kanałów).
export {
  fenceUntrusted,
} from './security/promptFence.js';
// Proweniencja wiadomości czatu (kto NAPISAŁ tekst tury) - bramka przywilejów człowieka.
export {
  HUMAN_MESSAGE_META,
...
// Slug tożsamości agenta (nazwa → fragment ścieżki na dysku), format
...
export {
    ...
    getTokenCount,
    calibrate,
    // Ta sama estymata ze STATYSTYK tekstu (długość + non-ASCII), żeby
...
// Wchłonięte z `src/utils/`. Oba pliki są czyste (zero `obsidian`), więc wchodzą
...
// Self-append: odczyt-najpierw WŁASNEGO pliku przed dopisaniem nowego wpisu (siostrzana
// wada `probeFile` - tam kandydaci nazw, tu treść znanego pliku do dokładki).
export { readIfExists } from './utils/vaultFs.js';
// Migrator plikowy - konsument poza core/ to `src/main.ts`.
// `migrateNamespace` (settings-namespace) NIE jest tu reeksportowany:
...
// Kanoniczny typ widoku czatu. Żyje w core/, bo `modules/chat/index.js`
...
// ── Silnik ustawień + zdarzeń ─────────────────────────────
// Magazyn ustawień runtime'u. `deepMergeMissing` żyje wewnątrz `core/runtime/`.
export { SettingsStore } from './runtime/SettingsStore.js';

// ── Transport HTTP (podklaster `core/http`) ──────────────
// Wszystkie te pliki MUSZĄ wstawać w gołym Node - `ObsidianHttpClient` dostaje
