...
export const en: Record<string, string> = {
  ...
  'mcp.search.param.scope': 'Without this field = the CURRENT agent memory (brain + sessions + summaries) — the default scope. "vault" = user notes — pass it explicitly when searching the vault (not memory). "memory" = the same as omitting the field, spelled out; requires the memory permission. A sub-agent without this field gets the user notes (it has no memory of its own).',
