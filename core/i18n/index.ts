...
/**
 * Słowniki to płaskie mapy `klucz → tekst` (celowo BEZ unii ~1900 kluczy — `t()` bierze
 * zwykły `string`, a nieznany klucz wraca sam do siebie).
 ...
 * @param {string} [locale] - wymuś konkretny locale (`'pl'`/`'en'` — per-agent
...
export function t(key: string, params?: Record<string, unknown>, locale?: string): string {
  ...
  if (params) {
    for (const [k, v] of Object.entries(params)) {
      // split/join zamiast replace(RegExp, String(v)) - wartosc parametru trafia do napisu
      // DOSLOWNIE (tekstowy zamiennik replace interpretuje $&, $`, $', $$), a przy okazji
      // zero kompilacji regexa na kazde wywolanie.
