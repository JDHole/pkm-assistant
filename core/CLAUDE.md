...
- `redactSecretValues` - redakcja `NormalizedError.message`/`details` PO WARTOŚCI znanego sekretu (`ctx.apiKey`), druga niezależna warstwa obok maski po kształcie/nazwie pola. Pure, ten sam plik co `normalizeError`. Szczegóły i pułapka kolejności z `maskSensitiveData` - gotcha 4e niżej.
- `redactSecretsDeep` - sama redakcja co `redactSecretValues`, ale na DOWOLNEJ wartości (`unknown`) i PRZED normalizacją - dla wołacza, który ma surowy obiekt błędu sprzed `normalizeError`. Ochrona przed cyklami (`WeakSet`). Pure, ten sam plik. Gotcha 4e.
...
`maskSensitiveData` maskuje nie tylko znane prefiksy kluczy (`sk-`, `gsk_`, `xai-`, `AIza`…), ale też **wartość pola o wrażliwej nazwie** (`Authorization`, `api_key`, `apiKey`, `x-api-key`, `*_key`, `token`, `secret`, `password`, `cookie`/`set-cookie`) - w JSON-ie, w nagłówku i w gołym tekście. Filtr po kształcie zawsze będzie spóźniony o kolejnego dostawcę; filtr po nazwie łapie także tych, których nie znamy. Próg 8 znaków chroni zwykły tekst notatek (`RetrievalEngine` też przepuszcza snippety przez maskę). Nie loguj obiektów żądań w całości - `core/utils/http_request.ts` wypisuje jedną linię z `httpLogSummary.ts` (metoda, adres bez query, status, czas, NAZWY nagłówków), a nie `JSON.stringify(request_params)`.

**`cookie`/`set-cookie` (bug B1, 2026-09-17):** proxy potrafi odbić nagłówek `Set-Cookie` w treści błędu dostawcy modelu. `normalizeError` (`core/utils/errorUtils.ts`) zostawia `details` CELOWO surowym obiektem (kontrakt adapterów - `t.is(details, err)`, patrz `errorUtils.test.ts`), więc jedyną warstwą osłaniającą taki nagłówek jest właśnie ta maska po nazwie pola, gdy `Logger.maskLogValue` przepuszcza `details` przez `JSON.stringify` → `maskSensitiveData` przed zapisem do `.pkm-assistant/logs/`. `SENSITIVE_KEY_RE` (goła nazwa/nagłówek: `cookie`, `set-cookie`) I `SENSITIVE_CAMEL_RE` (camelCase: `setCookie`) muszą znać `cookie`/`Cookie` tak samo jak `token`/`secret` - inaczej token sesji ląduje jawny w logu, limitowany wyłącznie capem długości linii w `LogFileSink` (500/200 znaków), nie maską. Dopasowanie jest po SUFIKSIE, świadomie: `cookie_consent`/`cookiePolicy` (pole OPISUJE ciasteczka, nie niesie ich wartości) zostają czytelne, tak samo jak `monkey`/`keyboard` nie łapią się pod `key`. **Nie dokładaj stripowania `SECRET_BEARING_FIELDS` do `details`** - to złamałoby udokumentowany kontrakt tożsamości (`normalizeError(err).details === err`) i podwoiłoby warstwę maskowania zamiast naprawić lukę w niej. Strażnik: `core/security/SensitiveDataGuard.test.ts` (sekcja „cookie / set-cookie w nazwie pola") + `core/utils/Logger.test.ts` (droga PLIKU logu przez `details`, nie tylko konsoli).
...
### 4e. Redakcja PO WARTOŚCI - druga warstwa obok masek gotchy 4c

`maskSensitiveData` maskuje po KSZTAŁCIE (prefiksy `sk-`/`gsk_`/…) i po NAZWIE pola - klucz API NIESTANDARDOWY (bez znanego prefiksu, albo krótszy niż próg wzorca), który wypłynie w WOLNYM TEKŚCIE bez rozpoznawalnej nazwy pola obok (nie `key:`/`token:`/`api_key=`…), przechodzi przez oba sitka bez zmian, jeśli dostawca albo proxy odbije go w treści błędu (`payload.error`/`raw.error` w `modules/models/providers/*.ts`). Filtr po nazwie pola łapie klucz pod wrażliwą nazwą niezależnie od kształtu - luka jest węższa niż "każdy nieznany klucz", dotyczy właśnie tego przypadku bez żadnej nazwy w pobliżu. `redactSecretValues(err: NormalizedError, secrets: readonly string[])` i `redactSecretsDeep(value: unknown, secrets: readonly string[])` (`core/utils/errorUtils.ts`, eksport z barrela) łatają TĘ lukę: zamieniają w tekście/obiekcie KAŻDE dosłowne wystąpienie sekretu NAPRAWDĘ użytego w żądaniu (`ctx.apiKey`) na `[REDACTED]` - w trzech wariantach (goły sekret, `encodeURIComponent(sekret)`, `Bearer <sekret>`, od najdłuższego, żeby forma opakowana nie zostawiła "Bearer [REDACTED]"). Próg 8 znaków jak w `maskSensitiveData` - chroni przypadkowe krótkie stringi. `redactSecretsDeep` różni się od `redactSecretValues` wyłącznie kształtem wejścia/wyjścia (`unknown` zamiast `NormalizedError`) - ta sama logika, ta sama ochrona cykli (`WeakSet` odwiedzonych obiektów, cykl → `'[cycle]'`).

**Kolejność jest zawsze ta sama, wszędzie: redakcja PO WARTOŚCI NAJPIERW, `maskSensitiveData` PO NIEJ - a jeśli wołacz ma SUROWĄ wartość sprzed `normalizeError`, redakcja idzie PRZED normalizacją, nie po niej.** `normalizeError` tnie `message` do `MAX_ERROR_MESSAGE_LENGTH` (4000 znaków) - klucz siedzący dokładnie na granicy cięcia wyszedłby przecięty na pół, gdyby redakcja czekała na gotowy `NormalizedError` (dopasowanie CAŁEGO sekretu już by go nie złapało). Stąd `redactSecretsDeep(raw, secrets)` PRZED `normalizeError(...)` wszędzie, gdzie wołacz zaczyna od surowego obiektu (`ChatModel.errorFromBody` gałąź JSON, `secureProviderError`/`secureAnthropicError`/`secureGeminiError` w providerach); `redactSecretValues` na WYNIKU `normalizeError` zostaje jako obrona w głąb (idempotentna - sekret już zniknął z tekstu). Redakcja po wartości zna dokładnie ten jeden sekret (klucz z TEGO żądania) i zamienia go na `[REDACTED]`; maska po kształcie/nazwie pola łapie wtedy inne, nieznane jej sekrety w tym, co zostało. `_maskValue` (`core/security/SensitiveDataGuard.ts`) zostawia bez zmian wartość, która JEST dokładnie tokenem `[REDACTED]` (dopasowanie DOKŁADNE po przycięciu białych znaków, nie podciąg), więc maska idąca po redakcji nie zjada go drugi raz - bez tego guarda `"bad key: [REDACTED]"` stałoby się `"bad key: [RED***CTED]"`, bo `key:` samo jest rozpoznawane jako wrażliwe pole. Guard jest świadomie WĄSKI: wartość, która token tylko NIESIE obok czegoś innego (`"[REDACTED] hunter2hunter2"`, `"[REDACTED]hunter2hunter2"` sklejone bez separatora) NIE jest samym tokenem i dostaje zwykłą maskę - inaczej prawdziwy sekret doklejony obok już zredagowanego tokenu przechodziłby jawny na plecach guarda. `OpenAiCompatibleProvider.ts`, `anthropic.ts`, `gemini.ts` (przy `ctx.apiKey`/`event.error`/`payload.error`) i `ChatModel.errorFromBody` (ciało HTTP błędu, obie gałęzie - JSON i nieparsowalny tekst) stosują ten sam łańcuch warstw. `ollama.ts` nie dostaje tej warstwy - `needsApiKey:false` w rejestrze, żaden nagłówek klucza nie idzie w żądaniu, więc `ctx.apiKey` nie niesie tam żadnej wartości do redagowania.
