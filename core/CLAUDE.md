...
**`cookie`/`set-cookie` (bug B1, 2026-09-17):** proxy potrafi odbić nagłówek `Set-Cookie` w treści błędu dostawcy modelu. `normalizeError` (`core/utils/errorUtils.ts`) zostawia `details` CELOWO surowym obiektem (kontrakt adapterów - `t.is(details, err)`, patrz `errorUtils.test.ts`), więc jedyną warstwą osłaniającą taki nagłówek jest właśnie ta maska po nazwie pola, gdy `Logger.maskLogValue` przepuszcza `details` przez `JSON.stringify` → `maskSensitiveData` przed zapisem do `.pkm-assistant/logs/`. `SENSITIVE_KEY_RE` musi więc znać `cookie`/`set-cookie` tak samo jak `token`/`secret` - inaczej token sesji ląduje jawny w logu, limitowany wyłącznie capem długości linii w `LogFileSink` (500/200 znaków), nie maską. **Nie dokładaj stripowania `SECRET_BEARING_FIELDS` do `details`** - to złamałoby udokumentowany kontrakt tożsamości (`normalizeError(err).details === err`) i podwoiłoby warstwę maskowania zamiast naprawić lukę w niej. Strażnik: `core/security/SensitiveDataGuard.test.ts` (sekcja „cookie / set-cookie w nazwie pola").
