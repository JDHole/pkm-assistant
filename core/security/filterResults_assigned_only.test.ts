/**
 * `AccessGuard.filterResults` gałąź A2 (agent w trybie „Tylko przypisane"
 * z PUSTYMI/undefined `focusFolders`, core/security/AccessGuard.ts:545-554). Bez testu na tę
 * gałąź mutacja ciała (zamiana na goły `return results;`) zostawiałaby pełny pakiet testów
 * zielony — `search`/`list` oddawałyby agentowi bez ani jednego przypisanego folderu CAŁY
 * zwykły vault.
 *
...
const VAULT_HITS = () => [
    ...
    { path: '.pkm-assistant/agents/jan/memory/brain.md' },
...
test('A2 blokuje: agent "Tylko przypisane" z PUSTĄ tablicą focusFolders nie dostaje ani jednej notatki zwykłego vaulta', t => {
    const agent: GuardedAgent = { name: 'Jan', permissions: { guidance_mode: false }, focusFolders: [] };
...
test('A2 blokuje: focusFolders undefined (nie tylko pusta tablica) traktowane identycznie jak pusta lista', t => {
    const agent: GuardedAgent = { name: 'Jan', permissions: { guidance_mode: false } };
    const filtered = AccessGuard.filterResults(agent, VAULT_HITS());
    t.deepEqual(
        filtered.map(r => r.path).filter(p => !p.startsWith('.pkm-assistant/')),
        [],
...
test('A2 przepuszcza: własna strefa robocza agenta (.pkm-assistant/) przechodzi MIMO pustych focusFolders', t => {
    ...
    t.deepEqual(
        ...
        [
            '.pkm-assistant/agents/jan/memory/brain.md',
...
test('A2: guidance_mode:true na tym samym agencie zachowuje CAŁY zwykły vault (kontrast — dowód, że A2 to osobna gałąź)', t => {
    const agent: GuardedAgent = { name: 'Jan', permissions: { guidance_mode: true }, focusFolders: [] };
    const filtered = AccessGuard.filterResults(agent, VAULT_HITS());
    t.true(
