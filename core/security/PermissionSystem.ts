...
/**
 * Kształt wpisu dziennika dostępu. Sam bufor (`accessLog`, `maxLogSize`, `logAccess`,
 * `getAccessLog`, `getAgentAccessLog`, `clearAccessLog`) został usunięty - zero odbiorców
 * w repo (ani UI, ani test). Typ zostaje jako eksport barrela, bo wartości TYPÓW nie
 * kasujemy przy sprzątaniu `core/index.ts`.
 */
 ...
 *
 * YOLO_MODE nie jest typem uprawnienia. „Nie pytaj” to tryb autonomii per-czat
 * (patrz core/security/autonomy.js), NIE uprawnienie agenta.
 */
// Pola-widma agenta (access_outside_vault/execute_commands/building_agents/
// system_settings/skills_crud) nie istnieją w katalogu. THINKING zostaje jako KATEGORIA AKCJI
 ...
 */
/**
 * Akcje, których `targetPath` NIE jest ścieżką vaultową.
 *
 ...
 *
 * `AccessGuard.checkAccess` z `targetIsVaultPath: false` wraca NATYCHMIAST
 * (`non-vault-target`), więc te ciągi nie są mierzone whitelistą `focusFolders`, No-Go ani
 * zakresem suba - bez tego agent w trybie „Tylko przypisane" traciłby wyszukiwarkę, pocztę
 * i delegację w całości. Reszta przepływu TUTAJ jest bez zmian: ryzyko, zgody i
 * `disabled_tools` dalej bramkują te akcje normalnie, a właściwe im granice pilnują ich
 * własne warstwy (rejestr URL, widoczność adresata, przecięcie zakresów delegacji).
 *
...
// Bez `export` - zero konsumentów poza tym plikiem; w tym pliku zostaje żywy w checkPermission().
const ACTION_PERMISSIONS: Record<string, string | undefined> = {
    ...
    // Artefakty żywe: create/update piszą TYLKO do folderu artefaktów - mapowane na
    ...
    // Narzędzia zewnętrznych serwerów MCP: akcja nie-vaultowa (bez path-checks AccessGuard,
    // wzór akcji web) - MCPClient nadaje pusty targetPath. Ryzyko RED liczy classifyToolRisk
    // z isExternalTool. Wpis MUSI istnieć jawnie, bo nieznana akcja = fail-closed deny
    // w checkPermission.
    'external.call': PERMISSION_TYPES.MCP,
    'web.search': PERMISSION_TYPES.WEB_SEARCH,
    // Pobranie strony to osobna akcja od wyszukiwania (osobna zgoda, osobny opis w modalu).
    // Typ uprawnienia zostaje wspólny - nie bramkuje.
    'web.read': PERMISSION_TYPES.WEB_SEARCH,
    'delegate': PERMISSION_TYPES.MCP
};

/**
 * Akcje, które MUSZĄ podać bramce cel.
 *
 ...
 * przeskakuje WSZYSTKO - np. `artifact_*` pisałoby do vaulta bez ani jednego strażnika, a
 * `add_text_to_image` zapisywałby pod `output_path`, którego bramka nigdy by nie zobaczyła.
 * Te akcje fizycznie dotykają plików, więc pusty cel = błąd wołacza = odmowa fail-closed.
 *
...
export const APPROVAL_DEFAULTS: Record<string, boolean | undefined> = {
    ...
    memory_save: true,
    web_search: true,
    // Własny klucz, domyślnie PYTAJ. Świadomie NIE dziedziczy ustawienia `web_search` -
    // user, który wyciszył wyszukiwarkę, nie zgodził się tym samym na wysyłanie danych
    // pod adres wybrany przez model.
    web_read: true,
    generate_image: true,
    // Wysyłka poczty do innego agenta - pytamy domyślnie (user widzi, co wychodzi).
    kom_send: true,
    // Żółte, domyślnie ciche.
    delegate: false,
    // Artefakty żywe piszą tylko do folderu artefaktów - domyślnie bez pytania (świadome).
...
export class PermissionSystem {
    // `declare` = sama deklaracja typu, zero emitu. Pola świadomie
     ...
     *
     * Tryb autonomii (`opts.autonomy`) wpływa TYLKO na `requiresApproval`
     ...
     * `yolo` nie przeskakuje hasPermission ani AccessGuard - znosi wyłącznie pytania.
     *
     ...
     * @param opts.scopeFolders - foldery `scope.folders` sub-agenta. Gdy
    ...
    checkPermission(
        ...
        // OBRONA W GŁĄB - ścieżka vaultowa idzie do formy kanonicznej ZANIM ocenią ją No-Go,
        // pliki chronione i AccessGuard. Kanonizacja dzieje się już u wołacza
        // (`MCPClient._extractToolContext`, żeby narzędzie dostało ten sam ciąg), ale bramka
        // nie ma prawa temu ufać - wołaczy jest więcej niż jeden.
        //
        // Akcje `image.*` mają tu też prawdziwe cele vaultowe: `generate_image` oddaje bramce
        // FOLDER ZAPISU z ustawień, a `add_text_to_image` - CEL ZAPISU (`args.output_path`
        // albo WYLICZONE `<źródło>_text.<ext>`). Cel wyliczany nie jest dosłownie wartością
        // żadnego pola argumentów, więc `_canonicalizeToolContext` u wołacza go pomija - bramka
        // jest dla niego JEDYNĄ warstwą kanonizacji. Bez tego ta sama ścieżka mogłaby dostać
        // dwie różne decyzje: `vault.write` + `./.pkm-assistant/x.png` → DENY, `image.generate`
        // + ten sam ciąg → ALLOW.
        //
        ...
        // dotykamy. `'/'` to umowny root listingu - zostaje bez zmian.
        ...
        // Akcja dotykająca pliku BEZ celu = odmowa fail-closed. Bez tego cały blok niżej jest
        // pomijany (stoi za `if (targetPath)`), a narzędzie i tak zapisuje - tyle że pod
        // ścieżką, której nikt nie sprawdził. `admin_access` NIE zwalnia: to nie jest granica
        // dostępu, tylko wymóg podania celu przez wołacza.
        ...
        // Uprawnienia akcji (read_notes/edit_notes/create_files/delete_files/mcp/web_search)
        // NIE bramkują - o „wolno" decyduje DOSTĘPNOŚĆ narzędzia (agent.disabled_tools,
        ...
        // yolo NIE omija whitelisty - znosi pytania, nie uprawnienia.
        if (targetPath) {
            ...
            // scopeFolders sub-agenta idzie do AccessGuard jako dodatkowa, KONIUNKCYJNA
            // bariera. Nie zastępuje żadnej reguły rodzica - dokłada się do nich.
            // Strażnik kanonizuje cel sam (obrona w głąb). Dla akcji `vault.*` / `image.*`
     ...
     *
     * Decyduje TRYB AUTONOMII (`autonomy`):
     ...
     *   pozycyjnych; żadna gałąź go już nie czyta)
    ...
    _getApprovalToggleKey(toolName: string | null): string | null {
        const map: Record<string, string | undefined> = {
            // Prymitywy (write/create_folder/...) mapują na te same klucze zapisanych ustawień
            // jak starsze narzędzia vault_write/vault_create_folder, żeby wcześniejsze
            // przełączniki approvalu usera dalej działały po zmianie nazwy narzędzia.
            'write': 'vault_write',
            ...
            'delegate': 'delegate',
            // Poczta agenta (YELLOW z własnym przełącznikiem w profilu → Uprawnienia).
            'kom_send': 'kom_send',
            // Delegacja jedzie pod przełącznikiem POCZTY, nie delegacji. Przełącznik ma
            // odpowiadać skutkowi: `agent_delegate` zostawia list w cudzej skrzynce dokładnie
            // tak jak `kom_send` (ten sam `sendAgentMail`), a przełącznik `delegate` (domyślnie
            // CICHY) opisuje bieg sub-agenta - zupełnie inną czynność. Skutek uboczny jest
            // zamierzony: user, który wyciszył pocztę, wycisza obie drogi.
            'agent_delegate': 'kom_send',
            // Artefakty żywe (toggle domyślnie OFF w APPROVAL_DEFAULTS). Aliasy remapują name
            // na todo/artifact_create PRZED approvalem, więc tu docierają już nowe nazwy
            // (artifact_*).
            'artifact_create': 'artifact_create',
            'artifact_update': 'artifact_update',
            // `todo` NIE jest read-side bez toggla - odczyt (get/list/read) jest GREEN
            // (autonomy.ts classifyToolRisk), ale zapis jest YELLOW i pyta wg WŁASNEGO,
