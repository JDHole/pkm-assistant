...
// `redactSecretValues` (`core/utils/errorUtils.ts`) redaguje po wartości PRZED tym, jak tekst
// dociera tu do maskowania po wzorcu - `key: [REDACTED]` ma wtedy zostać czytelny cały token,
// nie zamienić się w drugą, zagnieżdżoną maskę.

test('maskowanie: wartość niosąca już token `[REDACTED]` po nazwie wrażliwego pola zostaje bez zmian', t => {
    const payload = 'bad key: [REDACTED]';
    t.is(maskSensitiveData(payload), payload);
});

test('maskowanie: `token=[REDACTED]` zostaje bez zmian (separator `=`, nie `:`)', t => {
    const payload = 'token=[REDACTED]';
    t.is(maskSensitiveData(payload), payload);
});

// Guard w `_maskValue` chroni WYŁĄCZNIE wartość, która JEST dokładnie tokenem `[REDACTED]` - nie
// każdą wartość, która go w sobie NIESIE. Realny sekret doklejony obok już zredagowanego tokenu
// (odstępem albo bez separatora) ma dostać zwykłą maskę, nie przejechać jawny na plecach guarda.

test('maskowanie: sekret doklejony PO tokenie `[REDACTED]` (ze spacją) nie zostaje jawny', t => {
    const masked = maskSensitiveData('{"password":"[REDACTED] hunter2hunter2"}');
    t.false(masked.includes('hunter2hunter2'), masked);
    t.is(masked, '{"password":"[RED***ter2"}');
});

test('maskowanie: sekret doklejony PO tokenie `[REDACTED]` (bez separatora) nie zostaje jawny', t => {
    const masked = maskSensitiveData('token=[REDACTED]hunter2hunter2');
    t.false(masked.includes('hunter2hunter2'), masked);
    t.is(masked, 'token=[RED***ter2');
});

// `String.replace` ze stringiem jako zamiennikiem interpretuje `$&`/`$1`/`$'` w tekscie
// zamiennika - sekret zawierajacy taki wzorzec wracalby do wyniku w calosci. Zamiennik musi
// byc funkcja; te testy pilnuja, ze wartosc z `$&` dostaje zwykla maske 4+4.

test('maskowanie: sekret z `$&` w srodku nie wraca w calosci przez wzorzec zamiennika (pole=)', t => {
    const masked = maskSensitiveData('accesstoken=ab$&cdefghijkl');
    t.false(masked.includes('cdefghijkl'), masked);
    t.is(masked, 'accesstoken=ab$&***ijkl');
});

test('maskowanie: sekret z `$&` na poczatku nie wraca w calosci przez wzorzec zamiennika (pole:)', t => {
    const masked = maskSensitiveData('userpassword: Xy$&secretpart99');
    t.false(masked.includes('secretpart'), masked);
    t.is(masked, 'userpassword: Xy$&***rt99');
});
