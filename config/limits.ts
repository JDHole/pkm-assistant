/**
 * config/limits.js — Kagańce agentów
 *
 * JEDNO miejsce na twarde limity pętli narzędziowych agenta głównego i sub-agentów.
 * Bez tego stałe rozłażą się po plikach (chat: 10 iteracji, sub-agent: 3/5,
 * timeout delegacji: 60000ms, obcinanie wyniku narzędzia: 15000 znaków) i rozjeżdżają
...
export const DEFAULT_LIMITS = {
  ...
  // z domyślnym budżetem researcher-a (8). Twarda granica — model nie może jej ignorować.
  chat_max_iterations: 8,

  // Ile rund tool-callingu ma sub-agent (generyczny worker) w runAgentLoop (agent-loop).
  // Jeden worker dla wszystkich subów — brak osobnego limitu strategist.
  // 25, bo realna robota potrafi zjeść 12 iteracji i zostać zmuszona do backstopu w połowie
  ...
  // SubAgentRunner → runAgentLoop. Zegar ścienny NIE jest głównym strażnikiem — trupy (cisza
  // streamu) łapie watchdog ciszy (`subagent_stall_timeout_ms`); ten limit to awaryjny sufit
  // na bieg, który ŻYJE, ale mieli bez końca. 15 min, bo worker z 25 iteracjami na wolnym
  // moście (synteza sama trwa ~5 min) potrzebuje kilkunastu minut — krótszy sufit zabijałby
  // go w połowie roboty.
  delegation_timeout_ms: 900000,

  // WATCHDOG CISZY per wywołanie modelu SUB-AGENTA (ms). Timer przezbrajany
  ...
  // ścienny delegation_timeout_ms).
  subagent_stall_timeout_ms: 180000,

  // RATOWANIE DOROBKU (znaki). Gdy finalna synteza suba nie powstała (backstop
  ...
  // SUFIT WYNIKU SUBA przy doręczeniu do maina (znaki). Wynik suba to DELIVERABLE (deep
  // research po kilku minutach roboty), nie surowy zrzut narzędzia — bez osobnego sufitu
  // obcinałby się w połowie przy wspólnym `max_tool_result_length` (15k). Dotyczy OBU dróg
  // powrotu: powiadomienia z tła (buildSubTaskNotificationText) i wyniku narzędzia `delegate`
  // w pętli maina (per-tool override w AgentLoop). 0 = bez limitu.
  subagent_result_max_chars: 60000,

  // GRACE-OKNO na finalne podsumowanie suba (ms). `delegation_timeout_ms` obejmuje CAŁY
  ...
  // leci natychmiast, jak dotąd. 120 s, bo na wolnym moście lokalnym z modelem reasoning na
  // max effort jedna synteza z pełnym kontekstem realnie zajmuje ~2 min — krótsze okno
  // ubijałoby suby seryjnie w trakcie finalnej syntezy.
  ...
  // Strażnik GŁĘBOKOŚCI delegacji. Ile pięter delegacji wolno zbudować, licząc
  ...
  // Strażnik SZEROKOŚCI delegacji. Ile zadań wolno wsadzić do JEDNEGO wywołania
  ...
  // Max wysyłek per para nadawca→adresat w oknie 10 min. Strażnik siedzi w narzędziu
  ...
  // Sufit ŁAŃCUCHA auto-tur po subach z rzędu. Wynik suba
  ...
  // Default 10, nie 5: licznik liczy DORĘCZENIA, nie rekurencję,
  ...
  // DRUGI sufit — max wysyłek jednego NADAWCY w tym samym oknie
  ...
  // błędu), a i tak przerabia requesty po kolei, więc default 1
  ...
  // Ile znaków INSTRUKCJI custom suba (`config.prompt`, czyli KNOWLEDGE.md)
  ...
  // Ile znaków `context` przekazanego przez rodzica w `delegate` dociera
  ...
  // Watchdog czatu zbroi się od WEJŚCIA NA SLOT bramki
  // (gate_admitted), nie od zlecenia — czekanie w kolejce mostu za długą syntezą suba
  // nie jest ciszą modelu; inaczej tura ginęłaby po prostu za stanie w kolejce.
  chat_stream_stall_timeout_ms: 120000,

  // Budzik per-WYWOŁANIE modelu w CZACIE (pas ostateczny pętli; suby mają swój od dawna —
  // delegation_timeout_ms). Watchdog ciszy wyżej łapie brak chunków, ale NIE łapie promisy,
  // która nigdy się nie rozstrzygnie (XHR ubity z zewnątrz — transport strumienia nie emituje
  // zdarzenia na abort, więc pętla wisiałaby wiecznie). Kolejka bramki lokalnej się NIE liczy
  // (gate_admitted przezbraja budzik) — ale przezbrojenie jest JEDNO, więc faza kolejki też
  // musi się zmieścić w budżecie. 10 min, bo kolejka za syntezą suba na jednopasmowym moście
  // trwa realnie do kilku minut i pas ostateczny nie ma prawa jej ubić.
...
export const LIMIT_SPECS: Record<LimitKey, LimitSpec> = {
  ...
  // Sufit 100 — sub ma pracować aż skończy; strażnikami są watchdog ciszy i zegar zadania,
  // iteracje to tylko kaganiec na runaway-pętlę.
  subagent_max_iterations_worker:     { min: 1,    ceiling: 100,    integer: true },
  // Sufit 30 min — worker z podniesionym budżetem iteracji na wolnym moście potrzebuje
  // kilkunastu minut; krótszy sufit wiązałby ręce userowi.
  delegation_timeout_ms:              { min: 1000, ceiling: 1800000, integer: true },
  // Min 5000 — poniżej pięciu sekund grace-okno jest fikcją (jedno wywołanie modelu
  // z pełnym kontekstem tyle nie zdąży). Sufit 240000 — więcej niż 4 min na JEDNĄ syntezę
  // to już nie grace, tylko drugie życie. Nie ma tu 0 „wyłącz" — od tego jest krótki
  // `delegation_timeout_ms`.
  subagent_final_grace_ms:            { min: 5000, ceiling: 240000, integer: true },
  // 0 legalny (= watchdog ciszy wyłączony, zostaje zegar ścienny). Sufit = twardy
  ...
  // 0 legalny (= ratowanie dorobku wyłączone, stara zaślepka). Sufit jak
  ...
  // 0 legalny (= wynik suba bez limitu). Sufit jak wyżej — deliverable też
  ...
  // Min 1000 = instrukcji suba nie da się ściąć do zera (sub bez metody to nie sub).
  ...
  // Min 1000 (kontekst od rodzica ma sens dopiero od akapitu w górę), sufit 200000 —
  ...
  // Min 1 = delegacja zawsze musi być możliwa z poziomu agenta (0 wyłączyłoby
  ...
  // Min 1 (zawsze wolno jedno zadanie), sufit 20 — powyżej to atak na własne API.
  max_parallel_delegations:           { min: 1,    ceiling: 20,     integer: true },
  // Min 1 (poczta agentów nigdy nie jest wyłączana limitem — od tego jest
  ...
  // Min 1 (auto-tura po SUBIE zawsze może wystartować przynajmniej raz — to nie jest
  // wyłącznik funkcji, tylko kaganiec na ŁAŃCUCH, jak max_delegation_depth wyżej).
  ...
  // Sufit nadawcy stoi PONAD limitem pary, więc i jego widełki są szersze — min 1
