/**
 * Strażnik po źródle dla `sendInlineComment`.
 *
 ...
 *
 * Pułapka: `sendInlineComment` odmierzała gotowość widoku czatu GOŁYM `setTimeout(300)` —
 * wyłączenie pluginu w oknie budzika strzelało na zdemontowanym egzemplarzu. Naprawa: budzik idzie przez
...
test('sendInlineComment: budzik idzie przez registerInterval, nie goły setTimeout', t => {
    const body = methodBody(source, 'sendInlineComment');
    t.true(body.length > 0, 'nie znalazłem sendInlineComment w src/main.ts');
    // `window.` jest dopuszczone (wytyczna katalogu `prefer-window-timers`:
...
test('sendInlineComment: guard na widoku czatu przed dotknięciem input_area/send_message', t => {
    const body = methodBody(source, 'sendInlineComment');
...
/**
 * Updater został wycięty: katalog społeczności zabrania mechanizmów aktualizujących plugin, a
 * samo odpytywanie `api.github.com` co 3 h było jedynym ruchem sieciowym pluginu, którego nie
 * zaczął user. Ten strażnik pilnuje, żeby nie wrócił.
 */
test('updater nie wraca: zero odwołań do api.github.com i check_for_update', t => {
    const code = stripComments(source);
    t.notRegex(code, /api\.github\.com/, 'plugin nie ma prawa odpytywać GitHuba o wersje — katalog i BRAT aktualizują same');
    t.notRegex(code, /check_for_update\b/, 'metoda updatera została wycięta — nie przywracamy jej');
...
/**
 * Komendy i ikony wstążki rejestrują się w `onload()`, nie w `initialize()`.
 *
 * `initialize()` biegnie dopiero po `onLayoutReady` i czeka na `wait_for({loaded:true})`, więc
 * wywrotka bootu środowiska (albo `waitForLoaded`, które przy `unloading` NIGDY się nie
 * rozwiązuje) zostawiałaby usera bez JEDNEJ komendy w palecie i bez ikon na wstążce — a ikony
 * dodatkowo zmieniałyby pozycję na pasku (Obsidian układa je w kolejności rejestracji).
 * Rozwiązanie trzyma OBIE własności naraz: rejestracja w `onload()`, ale język znany wcześniej
 * z taniego, samodzielnego odczytu `.pkm-assistant/settings.json`.
 */
test('onload rejestruje komendy i wstążkę — nie initialize', t => {
    ...
    t.notRegex(initialize, /this\.registerCommands\(\)/,
        'rejestracja komend w initialize() uzależnia paletę od udanego bootu środowiska');
    t.notRegex(initialize, /this\.registerRibbonIcons\(\)/,
        'rejestracja wstążki w initialize() zmienia pozycję ikon u userów');
});

test('onload: setLocale idzie PRZED rejestracją komend', t => {
    const onload = methodBody(source, 'onload');
...
// ── rejestr embeddingu jest SLOTEM runtime'u, wstawia go composition root ──
...
/**
 * Sedno od strony USERA: gdy w ustawieniach stoi `language: 'pl'`, do `addCommand`
 ...
 */
test('nazwy komend są polskie, gdy zapisany język to pl', t => {
    const code = stripComments(source);
