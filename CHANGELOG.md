...
Honest failures and tighter keys: the chat reports what failed instead of claiming success, and
three security promises hold again after the base-layer rewrite.

### Fixed

- Manual compression and the token viewer presets report a failed summary instead of success.
- Attachments that are too many, too large or of an unsupported type show a notice with the reason.
- A refused clipboard write from the selection menu shows an error notice.
- The session save window shows a human sentence (interrupted, timeout, other error) instead of the raw error.
- The `/` trigger popup and its pending timer are closed when a chat tab closes or re-renders; the previous `@` autocomplete is destroyed before a new one is created; the note opener no longer retains a closed view.

### Security

- Provider error bodies and stream error events are redacted by the exact API key used in the request (plain, URL-encoded and `Bearer` forms) and then masked by pattern before they become a normalized error; the pattern mask no longer damages an already redacted token.
- A tool call naming an agent that no longer exists is refused instead of running as the active agent; a call with no agent at all is refused; approval windows are signed with the agent whose permissions were checked.
- The vault `.gitignore` protection compares whole lines and writes entries one after another, logging any entry that fails.
